...
---
This feature is to allow more flexibility to the user instead of having to remember specific command names.
The user can choose to use a different command name whenever he/she wants. This is useful when the user that needs to
type or user is lazy fast, they can choose to use shorter command names which takes slightly lesser time. It is also
useful for users who are forgetful.
...
** Add specific relationship groups that groups specific relationships together to enable easier viewing for grouping
 for user
...
** Updated architecture diagram for Model, Storage Component (Pull request: [][#])
