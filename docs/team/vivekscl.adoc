...
result he is looking for in the first place.
...
The change of window size is a event-driven command. When the user enters the command to change the window size,
the logic component executes the `ChangeWindowSizeCommand` which in turns posts a `ChangeWindowSizeRequestEvent`.
The User Interface component then handles event the `ChangeWindowSizeCommand` returns the success message.
The sequence of events can be summarised by the sequence diagram below.
