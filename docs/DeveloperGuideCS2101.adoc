...
**Pros:** Validation is not just done for user input file paths (ie. via `import` and `export` commands), but also +
for the address book file path as defined in the user's preferences (ie. `preferences.json`). +
