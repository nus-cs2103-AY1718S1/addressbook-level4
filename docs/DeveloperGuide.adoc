...
By: `Team CS2103-W13-B4`      Since: `Oct 2017`      Licence: `MIT`
...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th person in the ModU. The current state of the ModU is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).
...
We will pop the most recent command out of the `undoStack` and push it back to the `redoStack`. We will restore the ModU to the state before the `add` command executed.
...
. Add a `removeTag(Tag)` method. The specified tag will be removed from everyone in the ModU.
+
****
* Hints
...
. Modify the link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to show the total number of people in the ModU.
+
**Before**
+
...
** link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] needs to initialize the status bar on application start, and to update it accordingly whenever the ModU is updated.
* Solution
** Modify the constructor of link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to take in the number of persons when the application just started.
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the number of persons whenever there are new changes to the ModU.
...
. Add a new method `backupModU(ReadOnlyAddressBook)`, so that the ModU can be saved in a fixed temporary location.
+
****
* Hint
...
|`*` |user with many persons in the ModU |sort persons by name |locate a person easily
