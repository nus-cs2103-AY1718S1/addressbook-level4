...
allows the user to be able to view the list much easier by sorting them to whichever preference he/she wants/needs.

This enhancement is achieved by first creating a new SortCommand and SortCommandParser that processes the user input
command and calls the suitable method to achieve it. Since this enhancement is not able to reuse some of the method
in other components, new methods have to be created.
...
When viewing the persons list, user finds out if that specific person as my relationship with any other person. The
user is also able to set relationships between 2 persons if they are related to one another. Each person is able to
...
This will be done using the command `set [INDEX] [INDEX] [RELATIONSHIP]`.
