...
This feature works by updating the model's `filteredList` of `Person` objects with the predicate **isFavourite()** in `ListCommand`.
...
** The **Person** provided by the index has its `ProfPic` attribute's path var modified to **contactName.png** and
