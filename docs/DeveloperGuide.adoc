...
The *_Sequence Diagram_* _(Figure 2.1.2)_ below shows how the components interact using `EventsCenter` for the scenario where the user issues the command `delete 1`.
...
This results in a `Command` object which is executed by the `LogicManager`. Then, the command execution can affect the `Model` (e.g. adding a person) and/or raise events. And the result of the command execution is encapsulated as a `CommandResult` object which is passed back to the `Ui`.
...
* SaveS event storage data in xml format and read it back.
...
The following *_sequence diagram_* _(Figure 3.3)_ shows how the sort command works:
...
The following *_sequence diagram_* _(Figure 3.5)_ shows how the list event command works:
...
The following *_sequence diagram_* _(Figure 3.7)_ for executing a show participants command: `showP 3`. It will show all the participants of the third event in the current list.:
...
Below is a *_sequence diagram_* _(Figure 3.12)_ for executing a find event command: `findE first`. It will find events which contain keyword `first` in their names.

image::FindEventSequenceDiagram.png[width="800"]
_Figure 3.12: sequence diagram for find event command_
