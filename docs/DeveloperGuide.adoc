...
The facebook mechanism is facilitated by `FacebookCommandParser`, which resides inside the parser folder of the logic component.

The facebook mechanism searches a contact's email on Facebook as specified by the contact's index number, according to the last ordering of the contact list.

The `FacebookCommand` will inherit from `Command`.

When the command word `facebook` is typed into UniCity, `AddressBookParser` will call `FacebookCommandParser`.

`FacebookCommandParser` then takes in the command argument (index of the contact), wraps it as an `Index` class to be parsed into the `FacebookCommand` constructor.

Upon execution, the `FacebookCommand` posts an event to the `EventCenter` and the event will be dispatched to the `BrowserPanel` to load the search page.

The `BrowserPanel` has `loadPersonPage` and `loadPage` to load the url that holds the search page. The url consists of 2 parts: the first part is a constant url string that
exhibits the search behaviour on facebook while the second part will be the contact's email address to search on facebook.

[NOTE]
The user may be required to log in to Facebook in the browser panel of UniCity if that is his first Facebook command.

[NOTE]
This command may not work for some contacts as the specified contact may either not have a Facebook account or has not registered an account with the given email.

==== Design Considerations

**Aspect:** Facebook search by contact's name or email address +
**Alternative 1 (current choice):** The url in `loadPersonPage` to contain email address search +
**Pros:** Reduces ambiguity in comparison to searching by names as the chances of Facebook users sharing the same names or part of their names are higher compared to their email addresses. +
**Cons:** The users' contacts might not have registered a Facebook account under their specified email. In which case, the users will be unable to search for their desired contacts on Facebook. +
**Alternative 2:** The url in `loadPersonPage` to contain the contact's name search  +
**Pros:** With the contact's full name, there is a chance of finding the contact on Facebook. Accounts for contacts who do not have an account with the same email address. +
**Cons:** Facebook search by name may return a broader result as there are more Facebook users who share similar names. This contributes to an inaccurate result.
