...
When user calls for sorting to be implemented, an empty ArrayList would be created. Under the Model component, UniquePersonList would compare the names of all persons in lower case using a comparator and return an Observable List containing of ReadOnlyPerson.
...
When user calls for scheduling to be implemented, two arguments would have to be passed into the constructor.
...
When the user calls ‘schedule’ or ‘sche’ to schedule the contacts in cherBook, the Model will receive a schedule from the Parser.
...
**Pros:** After scheduling the contacts, the schedules remain in their order they are entered into the AddressBook. +
...
**Cons:** Additional space usage is needed so inefficiency and memory wastage may arise. +
**Alternative 2:** Adding a new attribute in each individual person card to hold the schedules. +
**Pros:** The teacher would be able to identify each student respectively. +
**Cons:** Inefficiency and time consuming because to find a schedule, the algorithm would have to iterate through all person cards to get the schedules.
// end::schedule[]
...
Aspect: Randomness of the assigned color. +
...
image::RemarkSequenceDiagramForLogic.PNG[width="350"]
