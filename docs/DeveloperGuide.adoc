...
* `DuplicateMeetingException`: Compares the editted Meeting object with the other Meeting objects in the address book
...
* `MeetingBeforeCurrDateException` : Exception is thrown when the date of the editted
Meeting is earlier than that of the current system time, this exception is thrown.
* `MeetingClashException` : Exception is thrown when the DateTime of the edittedmeeting is
...
**Pros:** More direct and faster way of editting the different parameters of the `Meeting` object +
...
image::MeetingCard1.png[align="center'][width="250"]
