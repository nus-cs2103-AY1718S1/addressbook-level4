...
In addition, the `MusicCommand` class will create a new `music thread`, this is to prevent the User Interface from freezing in the current thread. Currently, all music played will be in a loop until it is stopped by the user, through the `music stop` command.
