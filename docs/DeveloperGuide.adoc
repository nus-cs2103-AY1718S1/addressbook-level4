...
This List mechanism is facilitated by commands which uses `Logic` `Interface` to obtain the copy of the list that is required. As seen by the picture above, to obtain the current blacklist of the address book, "list" string is first captured by the `CommandBox`. The `CommandBox` then passes this string to the Logic interface for execution.
