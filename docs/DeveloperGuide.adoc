...
During the implementation, map command parser class extracts the index number of the contact and passes the value to the corresponding map command. For `m-route` command, the parser class also extracts your current location by using the prefix symble. The map command classes will post and raise an event once they are called, based on the parameters they have received.
