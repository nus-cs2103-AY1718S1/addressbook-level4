...
The `Import` mechanism allows users to import `Parcel`s from valid `Ark` storage files stored as `.xml` files into the
current instance of `Ark`. This mecahnism is facilitated by a `readAddressBook` method within`XmlAddressBookStorage` to
load the parcels stored in the `xml` file and an `addAllParcels` method defined in `ModelMnanager` to add the parcels in
...
`UndoableCommands` interface rather than inherit from the `Command` interface directly. +
