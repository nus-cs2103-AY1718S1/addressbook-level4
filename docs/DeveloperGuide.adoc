= RoomBook Level 4 - Developer Guide
:toc:
:toc-title:
...
:repoURL: https://github.com/se-edu/roombook-level4/tree/master
...
After forking the repo, links in the documentation will still point to the `se-edu/roombook-level4` repo. If you plan to develop this as a separate product (i.e. instead of contributing to the `se-edu/roombook-level4`) , you should replace the URL in the variable `repoURL` in `DeveloperGuide.adoc` and `UserGuide.adoc` with the URL of your fork.
...
[NOTE]
Note how the `Model` simply raises a `RoomBookChangedEvent` when the Room Book data are changed, instead of asking the `Storage` to save the updates to the hard disk.
...
.  `Logic` uses the `RoomBookParser` class to parse the user command.
...
* can save the Room Book data in xml format and read it back.

=== Common classes

Classes used by multiple components are in the `seedu.roombook.commons` package.
...
The undo/redo mechanism is facilitated by an `UndoRedoStack`, which resides inside `LogicManager`. It supports undoing and redoing of commands that modifies the state of the room book (e.g. `add`, `edit`). Such commands will inherit from `UndoableCommand`.

`UndoRedoStack` only deals with `UndoableCommands`. Commands that cannot be undone will inherit from `Command` instead. The following diagram shows the inheritance diagram for commands:

image::LogicCommandClassDiagram.png[width="800"]

As you can see from the diagram, `UndoableCommand` adds an extra layer between the abstract `Command` class and concrete commands that can be undone, such as the `DeleteCommand`. Note that extra tasks need to be done when executing a command in an _undoable_ way, such as saving the state of the room book before execution. `UndoableCommand` contains the high-level algorithm for those extra tasks while the child classes implements the details of how to execute the specific command. Note that this technique of putting the high-level algorithm in the parent class and lower-level steps of the algorithm in child classes is also known as the https://www.tutorialspoint.com/design_pattern/template_pattern.htm[template pattern].
...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th person in the room book. The current state of the room book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).
...
We will pop the most recent command out of the `undoStack` and push it back to the `redoStack`. We will restore the room book to the state before the `add` command executed.
...
The redo does the exact opposite (pops from `redoStack`, push to `undoStack`, and restores the room book to the state after the command is executed).
...
**Alternative 1 (current choice):** Saves the entire room book. +
...
**Alternative 1 (current choice):** Only include commands that modifies the room book (`add`, `clear`, `edit`). +
**Pros:** We only revert changes that are hard to change back (the view can easily be re-modified as no data are lost). +
**Cons:** User might think that undo also applies when the list is modified (undoing filtering for example), only to realize that it does not do that, after executing `undo`. +
**Alternative 2:** Include all commands. +
**Pros:** Might be more intuitive for the user. +
**Cons:** User have no way of skipping such commands if he or she just want to reset the state of the room book and not the view. +
**Additional Info:** See our discussion  https://github.com/se-edu/roombook-level4/issues/390#issuecomment-298936672[here].
...
The import mechanism is essentially adding multiple contacts to the room book automatically by referencing a xml file. It is implemented by calling add function repeatedly till all the new contacts have been added to the current contact list.

One consideration of this mechanism is that contacts with identical names will not be added as the current room book is regarded as the most updated version and the import feature aims to only facilitate addition of new contacts.
...
The sort command is implemented to sort the current Room book according to the following possible criteria:
* name
* phone
* room
* email
...
[NOTE]
We chose asciidoc over Markdown because asciidoc, although a bit more complex than Markdown, provides more flexibility in formatting. It supports adding of new contacts and will skip known contacts residing in the room book.
...
A project often depends on third-party libraries. For example, Room Book depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing. Managing these _dependencies_ can be automated using Gradle. For example, Gradle can download the dependencies automatically, which is better than these alternatives. +
...
** link:{repoURL}/src/main/java/seedu/room/logic/parser/RoomBookParser.java[`RoomBookParser`] is responsible for analyzing command words.
* Solution
** Modify the switch statement in link:{repoURL}/src/main/java/seedu/room/logic/parser/RoomBookParser.java[`RoomBookParser#parseCommand(String)`] such that both the proper command word and alias can be used to execute the same intended command.
** See this https://github.com/se-edu/roombook-level4/pull/590/files[PR] for the full solution.
****
...
. Add a `removeTag(Tag)` method. The specified tag will be removed from everyone in the room book.
+
****
* Hints
** The link:{repoURL}/src/main/java/seedu/room/model/Model.java[`Model`] API needs to be updated.
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/room/model/RoomBook.java[`RoomBook`] and link:{repoURL}/src/main/java/seedu/room/model/person/Person.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/room/model/RoomBook.java[`RoomBook`] allows you to update a person, and link:{repoURL}/src/main/java/seedu/room/model/person/Person.java[`Person`] allows you to update the tags.
* Solution
...
** See this https://github.com/se-edu/roombook-level4/pull/591/files[PR] for the full solution.
****
...
* Solution
** See this https://github.com/se-edu/roombook-level4/pull/592/files[PR] for the full solution.
****
...
** See this https://github.com/se-edu/roombook-level4/pull/593/files[PR] for the full solution.
****

. Modify the link:{repoURL}/src/main/java/seedu/room/ui/StatusBarFooter.java[`StatusBarFooter`] to show the total number of people in the room book.
+
**Before**
+
...
** link:{repoURL}/src/main/java/seedu/room/ui/StatusBarFooter.java[`StatusBarFooter`] needs to initialize the status bar on application start, and to update it accordingly whenever the room book is updated.
* Solution
...
** Use link:{repoURL}/src/main/java/seedu/room/ui/StatusBarFooter.java[`StatusBarFooter#handleRoomBookChangedEvent(RoomBookChangedEvent)`] to update the number of persons whenever there are new changes to the roombook.
** See this https://github.com/se-edu/roombook-level4/pull/596/files[PR] for the full solution.
****
...
. Add a new method `backupRoomBook(ReadOnlyRoomBook)`, so that the room book can be saved in a fixed temporary location.
+
****
* Hint
** Add the API method in link:{repoURL}/src/main/java/seedu/room/storage/RoomBookStorage.java[`RoomBookStorage`] interface.
** Implement the logic in link:{repoURL}/src/main/java/seedu/room/storage/StorageManager.java[`StorageManager`] class.
* Solution
** See this https://github.com/se-edu/roombook-level4/pull/594/files[PR] for the full solution.
****
...
. Modify link:{repoURL}/src/main/java/seedu/room/logic/parser/RoomBookParser.java[`RoomBookParser`] to accept a `RemarkCommand`.

**Tests:**

. Add `RemarkCommandTest` that tests that `executeUndoableCommand()` throws an Exception.
. Add new test method to link:{repoURL}/src/test/java/seedu/room/logic/parser/RoomBookParserTest.java[`RoomBookParserTest`], which tests that typing "remark" returns an instance of `RemarkCommand`.
...
. Modify link:{repoURL}/src/main/java/seedu/room/logic/parser/RoomBookParser.java[`RoomBookParser`] to use the newly implemented `RemarkCommandParser`.
...
. Modify link:{repoURL}/src/test/java/seedu/room/logic/parser/RoomBookParserTest.java[`RoomBookParserTest`] to test that the correct command is generated according to the user input.
...
. Modify link:{repoURL}/src/main/java/seedu/room/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `roomBook.xml` so that the application will load the sample data when you launch it.)
...
. Fix `validRoomBook.xml` such that the XML tests will not fail due to a missing `<remark>` element.
...
See this https://github.com/se-edu/roombook-level4/pull/599[PR] for the step-by-step solution.
...
|`**` |user |know the crash report so that when the roombook crashes |restore backup
...
|`*` |user |be able to export my roombook information  |I can import it into another device

|`*` |user |have tag coloring |find the category of contacts I am looking for

|`*` |user with many persons in the room book |sort persons by name |locate a person easily
...
(For all use cases below, the *System* is the `RoomBook` and the *Actor* is the `user`, unless specified otherwise)
...
2.  RoomBook shows a list of persons
3.  User requests to delete a specific person in the list
4.  RoomBook deletes the person
+
Use case ends.
...
** 3a1. RoomBook shows an error message.
+
Use case ends
...
.  Data files should be supported by previous version of room book (backward compatibility)
.  Data files should be portable to another computer with the same process running
.  Room book should backup every 30 days to aid disaster recovery
.  Query in the room book should be executed promptly
.  Room book should not take too much resources for other applications
