...
The timetable mechanism is combined panel of browser and timetable which can be toggled depends on the current view list.
In the lesson list view, the timetable will display the all lesson information about a module in a timetable format.
In the location list view, the browser will display the map of the location.
This mechanism is facilitated by using JavaFx.

[NOTE]
The timetable will only appear when the user is in the lesson list view.

image::timetableEg.png[width="900"]

In order to create a timetable using JavaFx, it involved the generating of data and generating of table.
...
The following link:#sequence-diagram[sequence diagram] shows how the timetable mechanism works:

image::timetable.png[width="900"]

==== Design Considerations

**Aspect:** Implementation of timetable +
**Alternative 1 (current choice):**  combine browser panel and timetable panel.
**Pros:** Toggle between the two panel easily. +
**Cons:** Takes up more memory than just single panel +
**Alternative 2:** Separate browser and timetable panel +
**Pros:** Update the specifications of each individual panel +
