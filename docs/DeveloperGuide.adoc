...
Alternatives considered is to allow user input multiple details in each search. It may help to narrow down and shorten the list of contacts that will be returned but it is not implemented as user may only recalled some details of a person only. i.e. user know the name but do not recall the number.
