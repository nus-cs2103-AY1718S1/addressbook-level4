...
Add event mechanism adds an event which stores in The Myth's Book,
which will then be used in a Calendar in future implementation. A reminder feature will also be tagged on to the event.

The add event mechanism is facilitated by an `EventList`, which is stored in The Myth's Book. Similar to the PersonList and TagList, the list contains the
...
The exectuion of DeleteEventCommand is processed in this way:
...
Figure 5.6.1 Theme List Command Diagram 1
...
Figure 5.6.2 Theme List Command Diagram 2
...
Figure 5.6.3 Theme List Command Diagram 3
...
As seen in Figure 5.13.1, when `favourites` is typed into the command line input, a new `FavouriteListCommand` is created.

image::FavouriteListCommandDiagram2.png[width="800"]
_Figure 5.13.1 Favourite List Command Diagram 2_
