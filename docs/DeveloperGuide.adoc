...
Google calendar request allows user to retrieve their google calendar information with a single command. The method uses
an OAuth API token, requiring a one-time login to the user's google account. Currently displays result in console.

==== Implementation

Uses the command word 'Calendar' or 'cal' to return the results. Implementation is the same as any other command, having the CommandWord parse through
'AddressBookParser', and then executes 'CalendarCommand'. The API token is stored in the 'client_secret.json' file under main/java/resources.
The login credentials are stored locally in the user/.credentials folder.

==== Design Considerations

**Aspect:** Requires internet access to retrieve calendar +
**Alternative:** Create a storage for previously retrieved calendar events +
**Pros:** Retains some functionality even when offline  +
**Cons:** Will take up unnecessary storage space in the application
