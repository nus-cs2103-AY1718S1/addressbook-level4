...
`SortCommandParser` takes in an argument in the form of `[PREFIX]/[ORDER]` that defines how `UniquePersonList` should be sorted, with `PREFIX` specifying the sort type and `ORDER` specifying the sort order. It first checks for validity against a regular expression. Once verified, the argument will be tokenized to identify the given prefix or order for how the `UniquePersonList` should be sorted. A `SortCommand` object is then created with the identified sort type and sort order.
...
For example, the user input might be `t-add college friend 1 2 3`.
After the command word `t-add` is parsed at the `AddresBookParser`, the remaining user input(tag to add, indices in the list), in the example `college friend 1 2 3`, will be passed to and parsed by `TagAddCommandParser`.
...
The key is to differentiate between command words and the indices to follow. The parser algorithm would first split the input string into parts by space as delimiter.
It would then determine whether a part is a word or a number, the parser keeps track of the last recognized word. After processing the entire user input, the parser would treat all parts after the last recognized word as indices and those before it as `tagName`.
In this case, as `friend` is the last part that is not numeric, `college friend` is treated as `tagName`; `1 2 3` is treated as indices.
