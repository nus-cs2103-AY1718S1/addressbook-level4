...
MedNus is designed for power users. Thus, batch deletions are a welcome addition since they make it less tedious to perform deletions on contacts.
...
Coupling between the logic and model component is minimised into the sole function call `deletePersonsByTags(Set<Tag> tags)`. This
is for simplicity as much as it is for ease of implementation. Future batch operations ought to be implemented
