...
the specific relationship between the 2 persons and also clear all relationships in the two persons. Each
person is able to have multiple relationships but only one relationship per person.

This feature is achieved by first creating the Relationship and UniqueRelList class that handles the relationship
 name and the list of relationship of each person. Next, creating the command to add, delete and clear relationship
between 2 persons.

This will be done using the command `set [INDEX] [INDEX] [PREFIX]RELATIONSHIP`. There are 3 different prefixes which
performs different tasks. *ar/* adds relationship, *dr/* deletes relationship and *cr/* clears all
relationships between the two persons.

*SetRelCommandParser* parses the input and returns a new SetRelCommand which has the parameters of the two indexes,
the new edited person and the boolean value of whether the add relationship prefix is present.

*SetRelCommand* executes and sets the relationship between 2 persons. It also checks if there is a mutual relationship
present in the indexes passed in. In order to change the relationship, the user will have to delete the
specific relationship and add the new relationship.

[NOTE]
TO-DO: Sequence diagram will be up soon.
