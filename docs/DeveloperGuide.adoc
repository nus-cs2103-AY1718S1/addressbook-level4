...
These commands will inherit from Undoable Command. Thus these commands are supported by undoing and redoing(using undo and redo command) of the current state of Addressbook.

This List mechanism is facilitated by commands which uses Logic interface to obtain the copy of the list that is required. As seen by the picture above, to obtain the current blacklist of the address book, "list" string is first captured by the Command Box. The Command Box passes this string to the Logic interface for execution.

Logic Interface uses Logic Manager, the class that implements its methods, to package it as a Command object. The respective command is then executed by this following statement in Logic Manager class:
...
Specifically for 'blacklist' command, it calls the getFiltererdBlacklistedPersons() method residing in the Logic interface. Logic Manager subsequently calls Model Interface. Model Manager handles the command and thereafter calls ReadOnlyAddressBook interface to handle the request. Addressbook handles this request and thus will return the required list, as the various lists are stored in this class. Technically, the list variable that exists in current structure is just persons. The blacklisted persons are obtained by running a check on all persons residing in this variable. This is shown by the following code below:
...
Although this seems inefficient, it supports syncing of the various lists well. Meaning, any changes that occurred in main list will be reflected in blacklist as well. For example, if a person is deleted from the main list, he will also be deleted from the blacklist. Thus, it is efficient in this aspect.
...
**Cons:** Implementation is complicated and ever changing whenever a new command is implemented. +
