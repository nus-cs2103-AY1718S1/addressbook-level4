...
_Figure 3.3.5 : Notifications via System Tray
...
_Figure 3.3.6 : Class Diagram for `Reminder`
...
_Figure 3.3.7 : Class Diagram for `Event`

image::EventReminderRS.png[width="300"]
_Figure 3.3.8 : Relationship of Event and Reminder
...
=== Sorting contacts mechanism

The sorting contacts mechanism uses the `sort` method which resides inside `List` interface to sort the contacts by name. It is called when adding a new contact (e.g. 'add` command), thus placing the contact in the list alphabetically by name.
