...
This enhancement builds upon the schedule feature developed in v1.3, which allows users to schedule an activity with one person. The code snippet from `ScheduleCommand` below shows how scheduling an activity with multiple persons is achieved.
