...
The remove feature allows users to either remove a `Tag` from a multiple persons, remove multiple `Tags` from
a single person or remove `Tag(s)` from the whole address book.
...
As seen above, execution of `RemoveCommand` is governed by the `removeTag` method located in the `Model` interface and implemented by
`ModelManager`.

==== Design Considerations
**Aspect:** Execution of remove command  +
**Alternative 1 (current choice):** Accepts multiple `Tags` and *one* `Index` or *one* `Tag` and multiple `Index` as a single command input. +
**Pros:** Allows greater efficiency in removing multiple `Tags` instead of doing it singly. +
**Cons:** May pose an inconvenience for users to remove `Tag` a wide range of `Index`. +
...
**Aspect:** Implementation of remove command. +
