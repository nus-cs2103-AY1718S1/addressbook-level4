...
=== Playing Music
Unify have a built-in background music player. Users have the choice of playing some music to relax their mind and reduce their stress.

The `music` command is extended with an additional command behind it. For example, `music play`, has play as the extended command, this will `play` command at the back means that the system will play a music track. While `music pause` will pause the current music track.

All these different actions are executed in the same `MusicCommand` class. This keep the commands easier to remember for the user as well, as anything related to `music` will start with `music` as a command.
...
----
[NOTE]
The `trackNumber` has to be equal or less than the number of each genre tracks you have in the `audio/music` folder. For example, if `trackNumber` is 2, it is required at least 2 mp3 files of each genre of music.

image::MusicList.jpg[width="300"]
...
image::MusicActivityDiagram.png[width="200"]
...
**Pros:** Easy to implement. Users able to know what song is coming up next in the track list +
**Cons:** Always start at the same music track first +
**Alternative 2:** Play a random music file +
**Pros:** Have a chance to be different the next time when first started playing music. +
**Cons:** User does not know which song is going to be played next. +

---

**Aspect:** Extended commands naming convention+
**Alternative 1 (current choice):** Extended commands are after the `music` command, hence `music play` & `music pause`+
**Pros:** Easier to remember those extended commands belongs to `music` +
**Cons:** Grammatically wrong.+
**Alternative 2:** Have the extended commands come first, hence `play music` & `pause music` +
**Pros:** Grammatically correct. Sounds nicer verbally. +
**Cons:** User might not associate those extended commands to `music`. +
