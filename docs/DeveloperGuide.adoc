...
If you would like to implement such a feature, please do so by adding additional paramters to the BatchCommand as such: +

 BatchCommand(TypeOfBatchCommand type, Set<Tag> tagsToDelete)

This is for reasons of complying with a Facade design pattern which is used throughout MedNus - particularly in cases where we want to retain polymorphism and support the Open-Closed principle. +
...
image::SDforCopyLogic.png[width="800"]
    Figure 23: Sequence diagram for a Copy command in the Logic component

As you've might have noticed in the diagram, two foreign-looking classes are used – namely, `Clipboard` and `ClipboardContent`.
These classes allow you to intuitively grasp how MedNus interacts with the system clipboard. Check out the
