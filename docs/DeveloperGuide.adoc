...
image::OpenAddressBookEvent.png[width="200"]

As you can see from the diagram, when the `Open` button is clicked in the Application, it raises an event and triggers
`MainApp` to update new Application metadata and restart the UI.
...
=== Add new person with optional fields

Adding new person in DeathNote requires only name. All other fields are optional. This is done by allowing the creation of
`Phone`, `Email`, etc. objects with `null` parameter.
...
. ParserUtil.java (Teach Deathnote how to interprete your new parameter)
...
. XMLAdaptedPerson.java (Ensure the XML file Deathnote keeps data in can recognise your parameter)
...
.Add code to "teach" Deathnote how to parse your parameter.Be sure to include examples
.Ensure that your parameter can be saved
