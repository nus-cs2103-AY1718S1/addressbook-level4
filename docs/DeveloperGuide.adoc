...
2. Take a look at the section link:#suggested-programming-tasks-to-get-started[Suggested Programming Tasks to Get
Started].
...
[NOTE]
Note how the `Model` simply raises a `ArkChangedEvent` when the Ark data are changed, instead of asking the `Storage`
...
.  `Logic` uses the `ArkParser` class to parse the user command.
...
* stores the data from interactions with Ark.
...
and redoing of commands that modifies the state of Ark (e.g. `add`, `edit`). Such commands will inherit from
`UndoableCommand`.
...
a command in an _undoable_ way, such as saving the state of the Ark before execution. `UndoableCommand` contains the
high-level algorithm for those extra tasks while the child classes implements the details of how to execute the
specific command. Note that this technique of putting the high-level algorithm in the parent class and lower-level
