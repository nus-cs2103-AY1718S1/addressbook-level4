...
The commands related to the Favourite system rely on the "Favourite" attribute class found in the Person class. name, email & address are some other attribute classes in a Person class.

==== Add & Remove contact from Favourites (Since V1.2)

**addFave:** This command's class accepts the parsed index of the contact from the current list of contacts displayed on the GUI. The contact's Person class is then retrieved and modified to have it's Favourite attribute marked as true and the in-memory model of the addressbook is updated with the modified Person.

**removeFave:** This command's class accepts the parsed index of the contact from the current list of contacts displayed on the GUI. The contact's Person class is then retrieved and modified to have it's Favourite attribute marked as false and the in-memory model of the addressbook is updated with the modified Person.
...
....
Travis-CI is a build automation tool that helps developers understand if their application is working by building, testing and reporting on the build sequences the developer has configured
....
