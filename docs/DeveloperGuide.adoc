...
**Pros:** Flexible level of precision according to the User and removes redundancy  +
**Cons:** Current data will not be updated if the Resident details are already entered in the ResidentBook +
**Alternative 2:** Add all entries found on the file specified by user +
**Pros:** +
**Cons:** This implementation may cause an error when searching for Residents. It also violates the ResidentBook Add command as there is duplicated person entries.
...
**Alternative 1 (current choice):** Saves the backup file to current location  +
**Pros:** Only require user to call function, with no additional details needed. Simple and straightforward  +
**Cons:** File name is fixed as "backup.xml" and user may need to rename it on the system hard drive +
**Alternative 2:** Allow user to specify backup file name +
**Pros:** Flexible level of precision according to the needs of the user +
**Cons:** May result in unnecessary backup with different names

One consideration for the choosen mechanism is that backup should only happen when necessary, as it will take up a lot of space.
...
**Cons:** Redundancy and overhead in keeping another list of Residents. Edit, Add, Delete will require additional checks to prevent outdated data.
...
This following section shows how the Tag specified is removed. It loops through the TagList each person has, and creates another set of Tags. While there will not be a duplication of tag, this defensive code ensures that indeed all specified tag is removed.
[source,java]
----
