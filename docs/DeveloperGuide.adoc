...
**Pros:** Retains the logic flow of how commands are executed. This option increases code reusability of the UndoableCommand interface and retains the cohesion of each class. +
...
**Pros:** Retains the logic flow of how commands are executed. This option increases code reusability of the Command interface and retains the cohesion of each class. +
...
**Aspect:** Choice of data type to represent the added contact image in `Photo` class +
...
**Cons:** Works in a very cumbersome way due to the two step conversion. +
...
**Cons:** Might present a problem if our team chooses to use alternative GUI generators that does not utilise Javafx scripting language. Image class might not be applicable to generate an Image.

---

**Aspect:** Choice of how the images are stored and accessed +
...
**Pros:** Image filepaths are now independent of the source filepath. Therefore users do not have to worry about shifting the specific image that is tied to a specified contact in PEERSONAL. +
...
**Alternative 2:** Directly obtains the local filepath of the image on the user's hard drive without writing the image into a new image storage folder `displaypicture` for PEERSONAL. +
**Pros:** Do not have deal with write issues to `displaypictures` and memory size of `displaypictures` if it gets too big. +
**Cons:** Users might forget that a specific image file is tied to a contact in PEERSONAL. Shifting the image file will cause PEERSONAL to be unable to find the image file. +
// end::addphoto[]
...
would be thrown. The following sequence diagram shows an example when `find n/...` command is executed:
...
will be executed. An instance of `Person` will be created to be updated with the birthday and returned to the `BirthdayCommand`.
...
It would then pass the Comparator to the Model component and executes the sort based on the Comparator using the inbuilt function
of `Collections.sort()`. It will then call the `updatedFilteredPersonList()` method in the `Model` class and display
