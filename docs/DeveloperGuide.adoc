...
// tag::deleteallphotos[]
=== Delete all photos mechanism
(added in v1.5)

The delete all photos mechanism is similar to the delete photo mechanism where it is also implemented by `EventsCenter` and `Logic`.

* After entering the command, the execute() method in `DeleteAllPhotosCommand` will delete all the photos in the project directory. +

==== Design Considerations
**Aspect:** Implementation of code +
**Alternative 1 (current choice):** Implementing this feature in a separate `DeleteAllPhotosCommand` class +
**Pros:** Separation of Concerns Principle (SoC) is not violated as the `DeleteAllPhotosCommand` class simply handles the deletion of all photos. +
...
is specifically concerned in deleting a person's photo while `DeleteAllPhotosCommand` is concerned with deleting all photos.
