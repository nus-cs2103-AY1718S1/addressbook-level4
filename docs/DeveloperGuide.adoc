...
Although delete remark is a separate command, it shares the same parser as Remark Command, according to different syntax, RemarkCommand Parser will either return a `RemarkCommand` or a `DeleteRemarkCommand`.
Adding a remark has the syntax `remark INDEX CONTENT` while deleting a remark has the syntax `remark -d INDEX`.
...
While the index in `remark -d INDEX` is the index of the remark we want to delete.

==== Design Considerations

**Aspect:** Implementation of DeleteRemarkCommand +
**Alternative 1 (current choice):** Use the same parser for RemarkCommand and DeleteRemarkCommand. +
**Pros:** The prefix of the command input for both of the commands is remark, it is more user-friendly since we summarise the two commands related to remark under the same prefix.
**Cons:** The parser has to choose to parse the input into one of the two commands according to the syntax.
// end::remark[]
