...
[NOTE]
The user can also choose to undo multiple commands at the same time. Sequence of commands to be undone for `undo 5` is the same as typing `undo` 5 times.

The redo does the exact opposite (pops from `redoStack`, push to `undoStack`, and restores the address book to the state after the command is executed). Similar to `undo`, user can also choose to redo multiple undo commands at the same time (`redo 5`).
...
. `AddressBookParser` calls upon `FavouriteCommandParser`, which parses the argument as an index and creates a FavouriteCommand object.
. The person identified by the index (personToFavourite) in `AddressBook` is replaced with a new person (favouritedPerson) that has its `favourite` instance inverted to either `true` or `false`.
...
The following diagram illustrates the sequence described above:
...
**Alternative 1 (current choice):** Favourited contacts are sorted separately. +
**Pros:** Favourite contact remains at the top of the list. +
**Cons:** Does not preserve the original order of the favourite contacts list. +
**Alternative 2:** All contacts sorted in order +
**Pros:** Easy to implement. +
**Cons:** Favourite contacts are no longer at the top of the list after sorting. +
**Alternative 3:** Favourite contacts are not included in sorting +
**Pros:** Preserves original order of favourites. Favourite contacts remain at the top of the list. +
**Cons:** User may want to sort favourites if list is large. +

---

**Aspect:** Unfavouriting contacts in a sorted list +
**Alternative 1 (current choice):** Contact removed from favourites is pushed to bottom of favourite list. +
**Pros:** Easy to implement. +
**Cons:** Does not follow the order of the sorting. User have to re-enter desired sort command after unfavouriting to get the correct order. +
**Alternative 2:** Unfavourited contact automatically sorted according to last used sort command +
**Pros:** Automatically sorts the list without having to type another command. +
**Cons:** User may not want the unfavourited contact to be sorted. +
// end::favourite[]
...
Codes below show how to parse base on options.
[source,java]
----
...
Switch mechanism just sends the `SwitchDisplayEvent` to `MainWindow`. It will change the child in `switchablePlaceholder` according to the user input.

The code below shows how switching is done:
...
**Aspect:** Handle different mode +
**Alternative 1 (current choice):** Use one command followed by mode string +
**Pros:** Easy to implement. +
**Cons:** Hard to remember. +
**Alternative 2:** Use two commands +
**Pros:** East to use and remember. +
**Cons:** More complicated to implement. Become hard to extend if there are more than two modes.
...
It takes in the user input name and compare it with all persons' name field. Similar to `find` by name, the user input name has to be the entire first/last/full name of the persons stored in the address book. Multiple contacts can be shortlisted by separating names by spaces.

As the command will always call the entire list before execution, there is no need to modify list for the command to be executed.

The code below shows how this command handles different input situations:
[source,java]
----
public CommandResult executeUndoableCommand() throws CommandException {
...
After parsing a valid user input name(s), the command will get a `filteredPersonList` using the parsed input:

* The contact is deleted immediately if there is only one result found.
* A updated list will be shown with multiple persons found.
* A warning message will be shown if no match can be found with the specified name.

==== Design Consideration

**Aspect:** Parsing of user inputs +
**Alternative 1 (current choice):** Convert the `String` user input into a `Predicate<ReadOnlyPerson>` object. +
**Pros:** Able to shortlist contacts with multiple names. +
**Cons:** Reject inputs that do not match the entire first/last/full name. +
**Alternative 2:** Utilise the given `String` user inputs +
**Pros:** Do not need to parse inputs before comparision. +
**Cons:** Able to delete contacts with a single letter or contacts with short name can not be deleted (set a minimum comparision `String` length, e.g. the input has to be at least 3 characters long).
// end::deletebyname[]
...
The add command is enhanced to allow user to leave certain fields empty (phone, address and email). Mechanism of the command is exactly the same as before, with the addition of a `initialiseEmptyFields` method in `AddCommandParser`. The `initialiseEmptyFields` method appends the prefix of each empty field followed by a '-' instead of throwing a `ParseException` if the fields are empty. Appending an empty field is done using the String.concat() method in the java.lang.String package. The method then returns a new string with the empty fields.

The code below shows how the `initialiseEmptyFields` method is implemented:
...
// tag::unredo[]
=== Enhance Undo/Redo command

The new `undo` and `redo` commands allows the user to specify how many commands to undo or redo (eg. `undo 5`, `redo 5`).
The sequence of commands to be undone for `undo 5` is the same as executing `undo` 5 times (refer to Section 3.2) . Similarly for `redo`, the number specified simply runs the `redo` command the same number of times. If no index is specified, the `undo` or `redo` command will only execute 1 time. `UndoCommandParser` and `RedoCommandParser` have been added in order to take in the index and `AddressBookParser` now calls upon `UndoCommandParser` and `RedoCommandParser` first instead of `UndoCommand` and `RedoCommand` directly.

The code below shows how the RedoCommandParser differentiate the index:
...
----
[NOTE]
The UndoCommandParser uses the same code. The only difference is that it returns an UndoCommand object instead.
// end::unredo[]
...
A project often depends on third-party libraries. For example, AcquaiNote depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing. Managing these _dependencies_ can be automated using Gradle. For example, Gradle can download the dependencies automatically, which is better than these alternatives. +
...
=== Use case: Undo/Redo multiple commands

*MSS*

1.  User keys in 4 undoable commands.
2.  Addressbook updates and displays updated data.
3.  User requests to undo 4 commands.
4.  Addressbook reverse the 4 most recent commands.
5.  User requests to redo 4 commands.
6.  Addressbook reverses the 4 most recent undo commands.
...
[none]
* 3a. No undoable commands in stack.
+
[none]
** 1a1. AddressBook displays message “No more commands to undo!”.
+
[none]
* 5a. No undo commands in stack.
+
[none]
** 5a1. AddressBook displays message “No more commands to redo!”.
+
Use case ends.
