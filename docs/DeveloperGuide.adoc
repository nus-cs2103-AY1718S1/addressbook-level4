...
`FindCommand` which reside in `Logic` will support the search of contact by any keywords.`Logic` uses the `AddressBookParser` class to parse user command.The find function only deals with existing parameters `Name`, `Email`, `Address`, `Phone`,`Tags` , `Activity` and `Country`. Such parameters inherit from `Person` which resides inside `Model`.Also,the keywords must be a full word match in order for successful execution of find command. Figures 4.4.1_ indicates the the parsing of user command in  `FindCommandParser` once user keys in an input.
...
Any other prefix as input will be considered as invalid input.In absence of valid prefix ,an exception will be thrown, as shown in _Figure 4.4.2_.
[source,java]
----
         default:
...
A successful match of the prefix will ensure that the keywords are  allocated to  method `containsKeyWordPredicate` under `Model`. Addressbook will then search for the keywords based on the revelvant parameters in `PredicateUtil'.
[source,java]
----
...
----
The component diagram in _Figure 4.4.3_ below shows how interactions within the Logic component for the execution of find command.

image::LogicComponentSequenceDiagramFind.PNG[width="800"]
_Figure 4.4.4 : Find component diagram_

Upon any successful match, the particulars of the contact will be displayed in `CommandBox`. _Figures 4.4.4a to 4.4.4g_ below indicate the various outcomes of search function by  using `FindCommand`.

image::FindCommand_Name.PNG[width="400"]
_Figure 4.4.4a : Find by name [n/NAME]_

image::FindCommand_Phone.PNG[width="400"]
_Figure 4.4.4b : Find by phone [p/PHONE]_

image::FindCommand_Email.PNG[width="400"]
_Figure 4.4.4c : Find by email [e/EMAIL]_

image::FindCommand_Address.PNG[width="400"]
_Figure 4.4.4d : Find by address [a/ADDRESS]_

image::FindCommand_Activity.PNG[width="400"]
_Figure 4.4.4e : Find by activity [act/ACTIVITY]_

image::FindCommand_Tag.PNG[width="400"]
_Figure 4.4.4f : Find by tag [t/TAG]_

image::FindCommand_Country.PNG[width="400"]
_Figure 4.4.4g : Find by country [c/COUNTRY]_
...
`CalendarCommand` which reside in `Logic` will  open up a new browser webpage in the `BrowserPanel` that shows calendar for the entire year. The webpage will display the calendar from January to December, inclusive of public holidays.
...
image::CalendarCommandComponentDiagram[width="800"]
_Figure 4.6.3 : Clenader sequence diagram_
...
The method `loadCalendar` will pass the calendar url to `BrowserPanel` to open up calendar website in `BrowserPanel`. The outcome of successful launching of calendar webpage in browser is shown in _Figure 4.6.5_:
...
`EmailCommand` utilise `Desktop` to launch a email platform for users to send email to multiple contacts.The sequence diagram of `CalendarCommand` in _Figure 4.9.1_ below indicates the sequence diagram of the execution of email function.

image::SequenceDiagramEmail.PNG[width="400"]
_Figure 4.9.1: Sequence diagram for email function_

User will first have to key in indexes of the contact. `Logic` will use `AddressBookParser` class to parse user command. `ParseException` will be thrown in `EmailCommandParser` when there is detection of illegal value.By using a `Set` to store the indices inputed by user, this will prevent duplication of indicies. Also, Figure 4.9.2_ below indicates the parsing of user input in `EmailCommandParser`.
...
A `Command` object will be executed by `Logic Manger`.During command excution, a filtered person list will be extracted from `Model`.
 The emails extracted from the filtered person list will be stored in a `recipientList` as shown in Figure 4.9.3_ .
...
'Desktop` class will be used to send email using `Desktop.Action.Mail` baed on recipientList.The outcome of successful launching of email platform to send email to multiple contacts is shown in _Figure 4.9.4:

image::EmailCommand.PNG[width="800"]

_Figure 4.9.4: Output of `EmailCommand.java` execution_


==== Design Considerations for `EmailCommand`


**Aspect:** Implementation of `EmailCommand` +
**Alternative 1 (current choice)**: Opening  `Calendar` webpage in `BrowserPanel`. +
**Pros:** To brings about greater convenience for the user as user does not need to navigate between multple webpages to find the calendar.User are able to find out about the public holidays which are not avaliable on desktop calendar. +
**Cons:** User might bypass the `CalendarCommand` and rely on desktop calendar +
**Alternative 2:** Opening up of google calendar +
