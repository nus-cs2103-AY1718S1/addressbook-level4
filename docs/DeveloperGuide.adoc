...
The `ScheduleCommand` takes in 2 parameters: `ScheduleDate` and `Activity`. Therefore, the Model component of the application has been extended to include these two new classes. `DateUtil` is created to make sure that users enter a valid data which follows DD-MM-YYYY format. The code snippet below shows how `DateUtil` checks the validity of an input date.
...
_Figure4.3.2: Interactions Inside the Logic Component for the `schedule d/15-12-2017 act/Meeting` Command_
...
This is an enhancement of the previously added feature of scheduling an activity with a single person.
==== Design Considerations
