...
**List command option:** The list command has been modified with the option to only show favourite contacts if you enter "list /f".
This feature works by updating the model's person filteredList with the predicate `isFavourite()` in `ListCommand.java`

==== Modifications to existing commands in regards to Favourite system (Since V1.4)

**List2 Command:** Favourite contacts will be loaded before all other contacts. Favourite contacts are pushed to the
top of the displayed list by using `model.sortPerson(comparator)` with comparator `getFaveComparator()`
// end::faveSystem[]

// tag::faveSystem[]
=== Profile Picture Feature
A user's profile picture is defined with the path stored in a contact's `ProfPic` person attribute and can be
changed with the use of 2 commands explain below.

==== Setting profile picture (Since V1.4)
`ppset`: The command first checks if the file given in the filepath is a valid .png file. Then it copies the file to
addressBook's resource/image folder and renames the file to the contact's name e.g. "eric.png" while setting the ProfPic
 attribute's path variable of the selected contact to point to the copied image file.

==== Resetting profile picture (Since V1.4)
`ppreset`: The command resets the contact's `ProfPic` attribute's path variable to point back to the stock photo's filepath.
// tag::faveSystem[]
