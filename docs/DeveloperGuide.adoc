= Sales Nagivator- Developer Guide
:toc:
:toc-title:
...
`LogicManager` proceeds to exectute the received command. In this implementation, the Command will call upon `ModelManager` to do complete the CRUD operations onto its internal database. (E.g. `AddEventCommand` calls `model.addEvent(event)` to add the event created by `AddEventCommandParser` to the event list in `AddressBook`. +
...
Whenever an event is created, it will be added into a `TreeMap` of
events, ranked by the scheduled date and time. This will enable us to
implement the event scheduling feature comprised of three essential
sub-features：

* A GUI displaying all the events scheduled for the current week
* A reminder feature to alert user of the upcoming events
* A scheduling check mechanism to prevent events with time clash to be
scheduled at the same time

==== Schedule for the week

When a `Check Schedule` command is typed into the command box, or the
`Calendar` button on the main window is clicked, a
`ShowCalendarEvent` will be sent to the EventsCenter. In response, the calendar
GUI will appear in the main window, and events with a scheduled `timing`
within the week will be identified from the `TreeMap` of events and reflected
 in the  calendar GUI. A corresponding List of events will also appear in the
  `EventListPanel`, just like with a `List` command for event.

==== Event Reminder

When the Sales Nagivator app is opened, a `ShowUrgentEvents` event will
automatically be sent, which will find all the events scheduled before the
end of the next day. After identifying these events, a small circle will
appear on the `notification` button on the main window to alert users of the
number of urgent events he/she should attend to. Upon clicking the
`notification` button, a list of urgent events will then appear in the
`EventListPanel`.

==== Avoid time clash

When a new event is added to the TreeMap, if the `timing` of the event has
any overlap with an existing event, this will generate a `ConflictingEvent`
exception that prevents the event from being added.

==== Design Considerations

**Aspect:** Date structure implementation of saved events +
**Alternative 1:** use an `ArrayList` +
**Pros:** Easy to implement and easy to access individual element +
**Cons:** Will be very slow for searching urgent events. +
**Alternative 2:** use a `HashMap` +
**Pros:** Very fast search (O(1)) if a good hash function is used +
**Cons:** Since a long period of time needs to be covered (at least 1 year),
the hash table size could be very big, which is not space efficient.
**Alternative 3 (current choice):** use a `TreeMap` +
**Pros:** Reasonably fast search operations +
**Cons:** Not as fast as a well-implemented `HashMap`.
...
When viewing the persons list, user is able to sort them according to `Name`, `Tags`, `Priority` or `Status`. This
allows the user to be able to view the list much easier by sorting them to whichever preference the user wants/needs.

This enhancement is achieved by first creating a new SortCommand and SortCommandParser that processes the user input
command that performs the suitable sorting method to achieve the user wants. Since this enhancement is not able to
reuse some of the method in other components, new methods have to be created.

The user input command will be passed to the SortCommandParsers which parses the input by checking to see if it is
empty. If the input string is empty, it will throw a *ParseException* which throws a new *ParseException* and displays
 to the user that the input format is incorrect and the usage of the command. If it is not an empty string, it will
 return a new instance of the SortCommand with the input as the parameter. SortCommand performs the
 executeUndoableCommand by calling the sortPerson method in the *ModelManager* class which will call the sortPerson
 method from the *AddressBook* class which class the method sortPersonList from the *UniquePersonList*. All of the
 parameter passed in in each method are the input string which indicates the type of sorting the user wants/needs.

In *UniquePersonList*, the string passed will then be checked using a `switch case`. If the type is found, it will
perform the corresponding sorting algorithm. If not it will throw a *InvalidSortTypeException* which tells the user
that the sorting type the user keyed in does not exist.

[NOTE]
Explanation of how this is achieved will be done soon. A sequence diagram will be updated here to illustrate how the
SortCommand and SortCommandParser operates.

// tag::Setting relationship[]
=== Setting relationship between two persons

When viewing the persons list, user finds out if that a specific person has a relationship with another person. Thus,
the user is able to set relationships between 2 persons if they are related to one another. The user can also delete
the specific relationship that the two persons are connected and also clear all relationships in the two persons. Each
person is able to have multiple relationships but only one relationship per person. The user will also be able to
edit the relationship of the two persons.

This feature is achieved by first creating the Relationship and UniqueRelList class that handles the relationship
 name and the list of relationship of each person. Next, creating the command to add/edit/delete relationship between 2
 persons.

This will be done using the command `set [INDEX] [INDEX] [PREFIX]RELATIONSHIP`. There are 3 different prefixes which
performs different things. *ar/* add the relationship, *dr/* deletes the relationship and *cr/* clears all
relationships between the two persons.

It will set the relationship for INDEX 1
and INDEX 2. Thus the command that executes this will perform twice.

The SetRelCommandParser parses the input and returns a new SetRelCommand which has the parameters of the two indexes
and the new edited person.

[NOTE]
More information on this will be done soon once this feature is almost complete.
