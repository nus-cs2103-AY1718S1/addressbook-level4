...
|`* * *` |user |undo a previous command |undo commands with typing mistakes
...
|`* *` |user |make groups for specific contacts |find grouped contacts faster
...
|`*` |user |mark a contact as "Favourite" |always find "Favourited" contacts easily
...
|`*` |user |have each contact display their profile picture |allow quicker identification of contacts with similiar names
...
|`*` |user |be able to customize the look and feel of the applicaiton |use the application with a comfortable UI

|`*` |user |link the application to online contact backup services |manage contact backups and synchronisation between devices

|`*` |user |have application and it's data file locked with a password |feel safe knowing my contact list will not be stolen and copied
