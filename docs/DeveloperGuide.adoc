...
_Figure 4.2 : Sequence diagram for `AddMultipleByTsvCommand`
...
_Figure 4.4 : Sequence diagram for `deleteMultipleCommand`_
...
By keying in the command word `recentlyDel`, the command word will be detected in the `AddressBookParser` class and then the `RecentlyDeletedCommand` class would be executed.
...
There are 2 themes to alternate from: `DarkTheme.css` and `LightTheme.css`. By keying in the command word `theme`, the command word will be detected by the `AddressBookParser` class and then the `ThemeCommand` class would be executed.
