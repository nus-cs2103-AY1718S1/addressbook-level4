...
The 3 parameters represents the index of the person in the AddressBook, a custom field name and a custom field value respectively.
...
For example, command `custom 1 Nickname Ahchuang` will add a nickname custom field with value "Ahchuang" for the first person in the AddressBook if nickname field previously does not exist.
...
For example, command `custom 1 Nickname` will clear the nickname custom field of the first person in the AddressBook.

==== Storage
Xml file for Custom Field is created and the Xml for Person is updated for storing the information of the customs fields of every person.

==== Ui
A new label will be displaced for each person for each custom field.

==== Sequence Diagram
image::CustomCommandSequenceProgram.png[width="800"]
...
The 2 parameters represents the index of the person in the AddressBook, the path of the photo image indented to upload.
...
For example, command `upload 1 D:\Git Projects\wolf.jpg` will add a photo image with path "D:\Git Projects\wolf.jpg" for the first person in the AddressBook if photo icon previously is the default.
...
For example, command `upload 1` will reset the icon photo of the first person to the default photo in the AddressBook.

==== Ui
Image of the icon photo will be displayed after uploaded.

==== Sequence Diagram
image::UploadPhotoCommandSequenceProgram.png[width="800"]

=== Search bar
==== Front end
The search bar is implemented in the front end using `SearchBox.fxml`, which controls the text field that receives query keywords from user.

At construction time, a buffer string is initialised to empty and is ready to store the current user input on the search bar.
...
Conversely, if a user hits backspace, a `KeyPressedEvent` is generated and captured in `handleKeyPress()`.
...
Like `CommandBox`, `SearchBox` has the same logic component to execute query words. Different from `#logic:execute(commandText)` that executes commands,
a dedicated method `executeSearch(searchText)` is used to parse the user input.
...
What is different is `SearchParser` returns either `FindCommand(Predicate)` or `ListCommand`,
depending on whether the searchText is empty - empty search word is interpreted as listing all persons, like other convenient modern address book search features behave.

The logic that executes commands returned by `SearchParser` is largely the same as before, with minor modifications.
Previously, a person would only be listed if an exact match of name is found; in this version, a person will be displayed as long as
all words in his full name contains all the search keywords as prefix.
For example, `Hansen Johannah` can be found by typing `Han Johan` in the search bar,
but `Hansen Jonannah` cannot be found because the prefix `Johan` is not in any part of the full name.
...
---
{stub to be copied and pasted later on for further entries}
...
=== More than one instance for each field
 (Coming in V1.3)

 ====Description

Since our target audience are the NUS students, which include a large propotion of international students, our addressBook will support the users to add multiple phone numbers, emails addresses and home addresses.

==== Model
UniquePhoneList, UniqueEmailList and UniqueAddressList Classes are created for each person to store the person's respective Fields.

UniquePhoneList is a list of Phone object. It will contain at least one local phone number as the primary phone number, this phone number is recognized by the country code +65.

This primary phone number must not be null, and it will be at the front of the UniquePhoneList.

While other additonal numbers are optional. Note that if the person has more than one local phone numbers, the one added first will be the primary number.

Likewise for UniqueEmailList and UniqueAddressList. The primary email will be NUS webmail and the primary address will be the local address.

Phone, Email and Address Classes are altered accordingly.

A Phone object now has two field, country code ("+" + two digits integer) and the actual number.

A Email object will be parsed in two parts, first part is the string before @ and the second part is after @, NUS webmail is recognized by the second part (.equals(u.nus.edu));

==== Logic
UpdatePhoneCommand, UpdateEmailCommand and UpdateAddressCommand will be created to add/remove these fields.

When a Update[sth]Command is created, it will be constructed with 3 parameters: Index, String, String.
The 3 parameters represents the index of the person in the AddressBook, action to be taken (add/remove) and a value respectively.

When action is add, we will create an object of the added instance and add it to the end of the Unique[sth]List.
When action is remove, we first check if the value equals to the primary value in the Unique[sth]List, this will flag an "primary [field] cannot be removed" message, otherwise we will just remove the value from the Unique[sth]List.


For example, command `updatePhone 1 add p/+8623333333` will add a phone number to the first person in the list of the addressBook.

command 'updatePhone 1 remove p/+6583060774' will first check whether p/+6583060774 is the primary phone number is the UniquePhoneList.
If yes, error messge "Primary phone number p/+6583060774 cannot be removed."
Else, p/+6583060774 will be removed from the UniquePhoneList.

Likewise for UpdateEmail and UpdateAddress.

==== Storage
Xml file for newly created list will be created and the Xml for Person is updated for storing the information of the fields of every person.
...
|`* * *` |user |back up the addressbook (save as...) |know where to look when addressbook is accidentally deleted
...
|`*` |user |share my addressbook with others |others can have my list of contacts
...
2.  AddressBook shows an example of add person message
...
2.  AddressBook displays the list of matched persons in the panel
+
Use case ends.
...
* 3b. Name given by the user is not in the AddressBook
+
[none]
** 3b1. AddressBook displays nothing.
+
...
2.  AddressBook shows a list of language supported
3.  User requests to change to a specific language in the list
4.  AddressBook changes to the chosen language
+
Use case ends.
...
.  Changes of the address book should be saved in a log file.
...
.  Address Book: The specific book that the user is using the application on.
