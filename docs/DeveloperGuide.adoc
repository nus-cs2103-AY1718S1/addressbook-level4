...
1) `Person` card selected from left panel

2) `Insurance` card selected from right panel

3) `Person` name selected from `InsuranceProfilePanel` when bound to middle panel

4) `Insurance` selected from `ProfilePanel` when bound to middle panel
...
These 2 events are picked up by `MainWindow` which rebind the middle panel placeholder to respective node.
[source, java]
----
 @Subscribe
