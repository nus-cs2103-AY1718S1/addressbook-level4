...
The sort command is executed by sorting the `internalList` in `uniqueLessonList` based on current `ListingUnit`.

[NOTE]
If the current `ListingUnit` is location, the list will be sort lesson's location lexicographically. +
If the current `ListingUnit` is module, the list will be sort lesson's module code lexicographically. +
If the current `ListingUnit` is lesson, the list will be sort lesson's group number lexicographically. +
...
In the CombinePanel, the handleViewedLessonEvent() will call generateTimeTableGrid() to set up the timetableView.  +
The generateTimeTableGrid() will obtain the grid data from generateTimeTableData(). +
...
`StickyNotes` is an enhancement of UI. The role of the stickynotes view is to display information about a module when. +
...
When the remark command is executed, a new note will be created and linked to a module. +
The handleRemarkChangedEvent() will call stickyNotesInit() to set up the stickyNotesView. +
The stickyNotesInit() will obtain the grid data from noteDataInit(). +
After that, it will display the data onto the stickyNotesView.
...
**Alternative1 (current choice): ** Using code to create stickynotes view. +
**Pros:** The color of stickynotes can be changed easily. +
**Cons:** THe developer needs to understand how to code for UI. +
**Alternative2:** Using Scene Builder to create stickynotes view +
**Pros:** It is easier to visualise the layout and set the properties +
**Cons:** There are many limitations such as unable to show only a few `TextArea` objects. +
