...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th lesson in the ModU. The current state of the ModU is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).

image::UndoRedoStartingStackDiagram.png[width="800"]

As the user continues to use the program, more commands are added into the `undoStack`. For example, the user may execute `add n/David ...` to add a new lesson.
...
**Alternative 1 (current choice):** Sort list by calling event to sort the current Observable<ReadOnlyLesson> lesson list on `LessonListPanel`. +
...
**Alternative 2:** Sort the actual `internalList` in `UniqueLessonList`. +
...
. Add a shorthand equivalent alias for each of the individual commands. For example, besides typing `clear`, the user can also type `c` to remove all lessons in the list.
+
****
* Hints
...
. Use different colors for different tags inside lesson cards. For example, `friends` tags can be all in grey, and `colleagues` tags can be all in red.
+
**Before**
+
