...
`AddPhotoCommandParser` will then return a `AddPhotoCommand` to be executed in the `LogicManager`. Upon the `execute()` method of `AddPhotoCommand`, a new `Person` object, `editedPerson` is created with all the same field as the `personToEdit` with the addition of the new `Photo` field. +
`PhotoStorage` is then called to rewrite the filepath of the image to a local folder `displaypictures` and return the new filepath to be set in `Photo` class. An error message will be thrown if PEERSONAL is unable to write the new image to the `displaypictures` folder. The `editedPerson` will take the new filepath of the image instead of the local filepath from the user input. +
...
image::SequenceDiagramUML.png[width="800]
...
**Cons:** `displaypictures` folder size might become too big if user adds too many photos. + 
...
**Cons:** Only able to find one contact at a time e.g. `find n/Alice Bob` does not work.  +
...
**Alternative 1 (current choice):** Allow sorting based on name, phone, email or address +
