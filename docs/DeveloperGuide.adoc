...
**Pros:** User can take note of contact's birthday more easily
...
The FindTag mechanism is facilitated by the `FindTagCommand` class which inherits from `Command` class. It allows the user to find persons using tags. Users can search using multiple tags too.

Suppose the user has just executed the `FindTagCommand` using `findtag friends`.

At first, the `execute()` function of the `LogicManager` class is called, which goes on to pass the command string into the `parseCommand()` function of the `AddressBookParser` class. This, in turn, determines that the `FindTagCommand` feature is being evoked and calls the `FindTagCommandParser` to parse the string and compare the tags. These are validated and formatted by the `TagContainsKeywordsPredicate` and returned as a `TagContainsKeywordsPredicate` object to be used to call the `FindTagCommand` class. The `LogicManager` then calls the `execute()` in the `FindTagCommand` class which creates and returns a `CommandResult` object.
...
Suppose the user has just executed the `TagCommand` using `tag 1,2 t/friend t/classmate`.
