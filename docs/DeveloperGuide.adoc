...
By: `Team The Myths`      Since: `Jun 2016`      Licence: `MIT`

== Introduction

The Myth's Book is an address book dedicated for university students to store useful information
about the people around them. The Myth's Book is a `Command Line Input` based software that is coded
...
=== Add Event
// tag::addevent[]
Add event mechanism adds an event which stores in the AddressBook, which will then be used in a Calendar in future implementation. A reminder feature will also be tagged on to the event.
...
The sort mechanism sorts the names of people inside the `addressbook` which
is inside the 'ModelManager'. It organizes the position of the people existing in
the addressbook based on their names in an alphabetical order. The sort command inherits
from UndoableCommand.

From Figure 5.2.1 below, it is visible that `SortCommand` inherits from `UndoableCommand`
which is an extra layer between abstract `Command` class in which the SortCommand
can be undoable, where the previous state of the address book before sorting
is saved.

image::SortCommandLogicClassDiagram.PNG[width="800"]
Figure 5.2.1 SortCommand Logic Class Diagram

Suppose a user has an unorganized list of contacts and executes the
`SortCommand` to organize his contacts based on their names in an
alphabetical order.

The execution of SortCommand is processed in this way:

. User will execute a new `SortCommand` so as to sort the address book.
. This is carried out by `model.sort()` in `executeUndoableCommand()`.
. The `internalList` of the address book is accessed and is sorted through a `lambda` function which
...
Sort method inside ModelManager.java :
[source,java]
----
@Override
...
Sort method inside UniquePersonList.java :
[source,java]
----
public void sort() {
...
When sort is typed into command line input, new SortCommand is created.
...
It will call the sort() function of its own class until it reaches InternalList.sort() where the sorting of contacts based on their names will begin.
...
**Pros:** Sort function call is able to be undone if needed as it now inherits from the Super Class. +
...
**Cons:** Unable to undo sort function and work from previous state of address book if user has called `SortCommand` to sort address book.
...
index of the desired contact in the contact list viewed. The `AccessCommand` class inherits from the `Command` class.

Suppose a user would like access one of his contacts website that is listed in the address book
and executes the `AccessCommand` to access the website.

The execution of AccessCommand is processed in this way:

. User will input `access index` where index refers to the index of the person listed in the person card list in GUI.
. Input of user will be parse as an argument through the AddressBookParser and AccessCommandParser to extract the index.
. A new `AccessCommand` with the index inputted will be executed.
. In the execute() of AccessCommand, the person of the website that the user wants to access will be obtained through the
 index of the `lastShownList` and stored into `ReadOnlyPerson` class variable.
. The `EventsCenter` will then post a new `AccessWebsiteRequestEvent` which contains the website of the person.
. The `AccessWebsiteRequestEvent` event would then be handled by an event handler `handleAccessWebsiteEvent`.
. The event handler will help to load the website of the person through the browser panel based on the website inputted.

Code of AccessCommand :
[source,java]
----
...
Sequence diagram for Logic Component of AccessCommand:
...
Sequence diagram for execute() of AccessCommand:
...
**Pros:** Able to have more features as AccessWebsite and Select feature is able to exist concurrently in the application. +
...
of creating a search on contacts name through he browser panel. +
...
The `find` command is an undoable command and therefore it inherits from `Command` rather than `Undoablecommand`.
