...
The `UndoableCommand` class follows the Liskov Substitution Principle because it fulfills `Command` 's
expectation of `UndoableCommand` objects. Substituting `DeleteCommand` 's objects for `UndoableCommand` 's objects
will not break the `Command` class functionality.
