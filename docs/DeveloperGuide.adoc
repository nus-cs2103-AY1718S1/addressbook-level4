...
This guide provides information that can help you improve the application in terms of coding and testing, and also allow you to better understand the features
...
Based on _Figure 2.1.1_, the App is made up of the following segments:
...
For example, in _Figure 2.1.2_ the `Logic` component defines it's API in the `Logic.java` interface and exposes its functionality using the `LogicManager.java` class.
...
The following diagrams _Figure 2.3.1_ and _Figure 2.3.2_ shows the structure of the logic component.
...
_Figure 2.4.1_ shows the structure of the Model Component
...
Lastly, _Figure 2.5.1_ shows the structure of the Storage component.
...
_Figure 3.1.1 : Sequence Diagram of the Email function_
...
As shown in _Figure 3.3.2_ below, the user executes a new `UndoableCommand`, `delete 5`, to delete the 5th person in the address book. The current state of the address book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).

image::UndoRedoStartingStackDiagram.png[width="800"]
_Figure 3.3.2: Undoable Command example_

As the user continues to use the program, more commands are added into the `undoStack`. For example in _Figure 3.3.3_, the user may execute `add n/David ...` to add a new person.

image::UndoRedoNewCommand1StackDiagram.png[width="800"]
_Figure 3.3.3: Undoable Command example_
...
We will pop the most recent command out of the `undoStack` and push it back to the `redoStack` as shown in _Figure 3.3.4_. We will restore the address book to the state before the `add` command executed.

image::UndoRedoExecuteUndoStackDiagram.png[width="800"]
_Figure 3.3.4: Undoable Command example_
...
_Figure 3.3.5_ : Sequence Diagram of the Undo nd Redo function._
...
_Figure 3.3.6: Undoable Command example_

Commands that are not undoable are not added into the `undoStack`. For example, `list`, which inherits from `Command` rather than `UndoableCommand`, will not be added after execution:

image::UndoRedoNewCommand3StackDiagram.png[width="800"]
_Figure 3.3.7: Undoable Command example_
...
_Figure 3.3.8: Undoable Command Activity Diagram_
...
* When all the contact in the file has been accessed and created as a `Person` object and stored inside the array, the `ImportCommandParser` will then return a new `ImportCommand()` will will parse the array to the `ImportCommand` class for execution.
...
then parses into `TagContainsKeywordsPredicate` which will override the test(ReadOnlyPerson) method where the method checks against the list of tags of each person and returns true for the person
that has the tags similar to the keywords. +
After which, the predicate returned by `TagContainsKeywordsPredicate` will then update the filtered list when it is parsed into the method updateFilteredPersonList(Predicate) which belongs to the `Model` class. +
...
The remove tag mechanism is implemented by `Logic` and `Model`. When parsing the command,each of the keywords inputted by the user is used to create a new Tag object that is put into a ArrayList<Tag>. +
...
and then removes the `Tag` that is parsed into the method. The setTags is then set for the newPerson and the updatePerson(oldPerson, newPerson) method is subsequently called to update the `Person` in the `AddressBook`.
...
_Figure 3.8: Remove Tag Sequence Diagram_
...
**Alternative 2 (current choice):** Keywords(case-insensitive) that are substrings or equal to the tag names will match +
...
**Alternative 2 (current choice):** Use an ArrayList<String> to store the keywords +
...
The changing of colour theme feature is added for users to choose their preferred colour theme. The colour theme options are implemented by the `UI`
...
will trigger the onAction method "handleWhiteTheme". The method adds the stylesheet to the `VBox` that is specific for the colour theme chosen and removes the stylesheets
that belongs to other colours. +
The diagrams shows the UI of the additional colour themes:

image::whiteThemeUI.png[width="800"]
_Figure 3.9.1 : White Theme_

image::greenThemeUI.png[width="800"]
_Figure 3.9.2 : Green Theme_
...
|=======================================================================
|Priority |As a ... |I want to ... |So that I can...
|`* *` |user |filter the list through details such as names |search for those contacts that I only need.
