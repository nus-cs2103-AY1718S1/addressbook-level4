...
* User is currently able to use `Person Commands` such as the `AddCommand` or `EditCommand`.
* User is currently unable to use `Event Commands` such as the `AddEventCommand` or `DeleteEventCommand`.
* User is able to use `General Commands` such as the `HelpCommand` or `SwitchThemeCommand`.
...
. User is now able to use `Person Commands` such as the `AddCommand` or `EditCommand`.
. User then uses the `AddCommand` and inputs `add n/Dion Poh p/63425234 e/dionPoh@hotmail.com`.
. Input of user is successfully parse as an argument through the `AddressBookParser` and then through the `AddEventCommandParser` and adds a new contact successfully.
