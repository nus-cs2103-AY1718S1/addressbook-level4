...
image::SDforTagColor.png[width="800"]
    Figure 18: Sequence diagram for `random tag color` Model Component
...
MedNus is designed for power users. Thus, batch deletions are a welcome addition since they make it less tedious to perform deletions on multiple patients, as grouped by tag.
...
image::SDforBatchDeleteLogic.png[width="800"]
    Figure 23: Sequence diagram for a batch myTags command in the Logic component
...
The user can occasionally clean up their contacts in case duplicates have accumulated over time. The `duplicates` command compiles a list of potential duplicate contacts for the user to review.
...
image::SDforDuplicatesLogic.png[width="800"]
    Figure 23: Sequence diagram for a duplicates command in the Logic component
...
image::SDforDuplicatesModel.png[width="800"]
    Figure 24: Sequence diagram within the Model component resulting from a duplicates command
...
This predicate is compared against persons in the address book, so as to generate a list of potential duplicate persons.
At the moment, persons are considered potential duplicates if they share the same name (case-sensitive) with any other person in the address book.

*Future enhancements*

We would like to include a sync feature with a user's online accounts (e.g. Facebook, iCloud) that would increase the ability of MedNus to resolve duplicates on the user's behalf.
// end::dups[]
