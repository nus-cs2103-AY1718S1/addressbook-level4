...
image::LogicCommandClassDiagram.png[width="800"]
_Figure 3.1.1: Inheritance diagram for Undoable Command_
...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th person in the address book. The current state of the address book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command). This is shown by Figure 3.1.2a.

image::UndoRedoStartingStackDiagram.png[width="800"]
_Figure 3.1.2a: Starting diagram for UndoRedoStack_
...
_Figure 3.1.2b: Adding a new command to UndoRedoStack_
...
We will pop the most recent command out of the `undoStack` and push it back to the `redoStack`. We will restore the address book to the state before the `add` command executed. This is shown by Figure 3.1.2c.
...
_Figure 3.1.2d: Sequence diagram for undo_
...
Commands that are not undoable are not added into the `undoStack`. For example, `list`, which inherits from `Command` rather than `UndoableCommand`, will not be added after execution. This is shown by Figure 3.1.2f.
...
_Figure 3.1.2g: Activity diagram for UndoRedoStack_
...
_Figure 3.3.1: Sequence Diagram of backing up the address book data_
...
// tag::list[]
=== List (Mainlist, Blacklist etc) mechanism

List feature is a useful feature to view persons of a certain category. In the context of this project, master list contains all the contacts of the debt collection agency.

We are displaying multiple lists to suit the need for the direction of the project. The lists could be viewed with the respective commands that will bring it to picture. The commands to materialize these lists are named after the lists itself. Meaning, to view the blacklist, the command typed is "blacklist" or "bl'.

These commands will inherit from `UndoableCommand`. Thus, these commands are supported by `undo` and `redo` command residing in the current state of Addressbook.

This List mechanism is facilitated by commands which uses `Logic` interface to obtain the copy of the list that is required. As seen by the diagram 3.2.1 and 3.2.2 above, to obtain the current blacklist of the address book, "list" string is first captured by the `CommandBox`. The `CommandBox` then passes this string to the `Logic` interface for execution.

`Logic` interface uses `LogicManager`, the class that implements its methods, to package it as a `Command` object. The respective command is then executed by this following statement in `LogicManager` class:
...
Specifically for `blacklist` command, it calls the `getFiltererdBlacklistedPersons()` method residing in the `Logic` interface. `Logic` `Manager` subsequently calls `Model` interface. `ModelManager` handles the command and thereafter calls `ReadOnlyAddressBook` interface to handle the request. `AddressBook` class handles this request and thus will return the required list, since the various lists are stored in this class. Technically, the `List` `Object` that exists in the current structure just contains all persons. The blacklisted persons are obtained by running a check on all persons residing in this variable. This is shown by the following code below:
...
Although this seems inefficient, it supports syncing of the various lists well. Meaning, any changes that occurred in main list will be reflected in blacklist as well. For example, if a person is deleted from the main list, he will also be deleted from the blacklist. This also works for various other commands. Thus, it is efficient in this aspect.
...
Figure 3.5.1 : Sequence Diagram of How the Blacklist Command Works_

The following sequence diagram, Figure 3.5.2, shows further details of the interaction between `Model` and 'Addressbook` to obtain the blacklist:

image::BlacklistCommandSequenceDiagram2.png[width="800"]
Figure 3.5.2 : Sequence Diagram of Model and Addressbook Interacts To Generate The Blacklist From All Persons_
