...
When the user enters any `Command` that modifies the Addressbook,
a `FilteredPersonListChangedEvent` containing the current `filteredPersonsList` from the `model` is created and posted to the `EventsCenter`.
...
The Labels in the User Interface are set to their corresponding values calculated from the `Statistics` instance's getter methods.
