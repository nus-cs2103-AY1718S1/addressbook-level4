...
5. The application displays the results in the specified sort order.
...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `rolodex.xml` so that the application will load the sample data when you launch it.)
...
. Fix `validRolodex.xml` such that the XML tests will not fail due to a missing `<remark>` element.
...
|`* * *` |advanced user |perform actions with hotkeys |increase productivity

|`* *` |user |hide link:#private-contact-detail[private contact details] by default |minimize chance of someone else seeing them by accident

|`* *` |user |delete a specific tag |remove tags that I no longer need

|`* *`|user |store notes about a contact|remember things pertaining to the contact

|`* *` |user |use abbreviation when typing commands |type commands faster

|`* *` |user |be prompted for confirmation when clearing the rolodex |avoid accidentally clearing the rolodex

|`*` |user with many persons in the rolodex |sort persons by name |locate a person easily

|`*` |user with many persons in the rolodex |delete a person using their name |remove a contact without having to find their index
...
.  Features of the product should be added incrementally.
.  Should be able to work offline.
.  Should be free to download.
.  Startup time should be less than 3s.
.  Should be usable by a novice who has never used a command prompt before
.  Data should be encrypted when application is not running
.  A computing professional should be able to manage his/her Rolodex contacts in a fast and efficient manner
