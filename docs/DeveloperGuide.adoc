...
a command in an _undoable_ way, such as saving the state of the address book before execution. `UndoableCommand` contains the
...
It takes in a parcel's postal code number by `parcel.getAddress().postalCode.value.substring(1)` and concatenates it
to the back of `GOOGLE_MAP_URL_PREFIX` to get a URL for the browser to load.
...
**Pros:** Its easy to implement a new method to load a new URL since it is a simple browser enhancement
to display a different URL +
**Cons:** It becomes +
**Alternative 2 (future choice):** Implementing through Google Maps API +
**Pros:** Makes it easier for implementing additional features that utilizes the Maps API such
 as drawing a route between two locations +
**Cons:** More difficult to implement and integrate into a command line interface, and its features are currently
 unnecessary for the current functionality of Ark.
