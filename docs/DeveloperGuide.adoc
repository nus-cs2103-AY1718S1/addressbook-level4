...
session. +
* **Alternative 2:** Backup data every few minutes +
...
**Aspect: Case: Backup if safe file is not present** +
...
* **Alternative 2:**Back up even if the save file does not exist or cannot be read by Ark. +
...
The Postal Code field is implemented as part of Address. The `Postal Code` class stores the postal address of locations
in Singapore. It only accepts values of `s` or `S` followed by 6 digits. The value stored in this class is used to store
 the postal code of the address. The value is used to query Google Maps when the `select` command is executed.
...
** **Pros:** This composition relationship is more intuitive. If an `Address` is deleted, its corresponding `Postal
...
`Status` is implemented using `Enumeration` interface and it has the four possible values:

image::StatusClassDiagram.png[width:"250"]
...
* **Alternative 1 (current choice):** Status implements an Enumeration.
...
** **Cons:** More overhead. +
* **Alternative 2:** Update `State` in `getInstance()`.
...
* **Alternative 1 (current choice):** Tag is implements the Enumeration class.
