...
_Figure 4.5.2 : Messages shown when the user does not enter the optional fields_
...
_Figure 4.6.2 : Color Range of Hue, Saturation and Luminance_
...
_Figure 4.6.3 : Compatibility of tag color background with white font of tag description_
...
**Alternative 1 (current choice):** Create a new `FilteredPersonListChangedEvent` with the responsibility to update the `Statistics Panel`. +
**Pros:** Adherence to the Single Responsibility Principle (SRP) increases cohesion among the different `Events` and reduces coupling in the source code. +
**Cons:** Reduces the number of classes in the source code, making it less complex. +
**Alternative 2:** Modify `AddressBookChangedEvent` to also update the `Statistics Panel`. +
**Pros:** Since `AddressBookChangedEvent` and updating the `Statistics Panel` are both triggered by a change in the model,
only a few changes to `AddressBookChangedEvent` are needed for it to also update the `Statistics Panel`. +
