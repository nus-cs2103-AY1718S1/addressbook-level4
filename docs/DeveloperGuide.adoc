...
Adding new person in DeathNote requires only name, other fields are optional. This is done by allowing the creation of
`Phone`, `Email`, etc. objects with `null` parameter.

Problem arises when these field class can create with `null` parameter. Necessary changes are mostly about additional
