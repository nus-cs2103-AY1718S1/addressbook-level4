...
The `BorrowCommand` class extends `UndoableCommand` so that this action can be undone or redone if necessary.

The borrow command requires the index (as listed in the person list panel on the left side of the application window) of the person who borrowed money and the amount that the person borrowed. To tokenize the input, `String::split` method is used in `BorrowCommandParser` class since no prefixes are used and the arguments (index and amount borrowed) are separated by a single whitespace. The index and amount borrowed are parsed and supplied to `BorrowCommand` as arguments as seen in Figure 3.2.1.
...
**Cons:** Requires modifications since using whitespace as a prefix and tokenizing arguments using `ArgumentTokenizer::tokenize` method causes incorrect tokenizing of arguments.
          For example:
...
**Alternative 1 (current choice):** Create a new `Person` object, called `editedPerson`, by copying all the attributes from the target `Person` object and update the debt field before replacing the old `Person` object with the `editedPerson` object. +
**Pros:** Straightforward and simple to implement +
**Cons:** Have to create a copy of all the attributes, even though debt is the only attribute to be updated, in order to create a new `Person` object to replace the old one. +
**Alternative 2:** Reusing the `updatePerson()` method in `AddressBook.java` +
**Pros:** Do not have to code the method from scratch +
**Cons:** Have to write more code to convert the `editedPerson` `Person`object to be of type `ReadOnlyPerson` because the `updatePerson` method only accepts `ReadOnlyPerson` objects as parameters. +
