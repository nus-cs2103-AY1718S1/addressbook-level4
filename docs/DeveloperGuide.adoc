...
`PersonPanelSelectionChangedEvent` is posted to the `EventBus`. _Figure 4.3.7_ below shows the full UI outcome of selecting a selecting a `PersonCard` in the `PersonListPanel`, as well as displaying the corresponding address location in `BrowserPanel`.

image::Full UI outcome for LocateCommand.png[width="800"]
_Figure 4.3.7 : Full UI outcome of locate implementation_
...
Upon any successful match, the particulars of the contact will be displayed in `CommandBox`. _Figures 4.4.3a to 4.4.3e_ below indicate the various outcomes of search function by  using `FindCommand` .
