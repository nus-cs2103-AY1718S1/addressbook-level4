...
The `removeTag` command is to remove a tag with name specified from address book. All the appearances of the tag in
...
**Alternative 1 (current choice):** The command implemented using methods which remove the tag +
from each person's tag list. +
**Pros:** Implementation is modular and less dependent on methods used for other commands. +
**Cons:** More tedious to implement since it is not making use of the existing resources such as +
`edit` command. +
**Alternative 2:** Adapt current `edit` command and removing the tag is done by editing every person's tag list. +
...
**Cons:** As the removal of Tag is from all the persons in address book, an accidental removal can be disastrous if +
the user does not intend a complete removal.

--

**Aspect:** Removal of the tag from all instead of some by user's choice +
**Alternative 1(current choice):** The command currently does not limit the removal of the tag from the persons +
which can be specified in user input. The command currently removes the tag from all persons in the address book. +
**Pros:** Easier implementation since it does not deal with the parsing of more user input. +
**Cons:** Inefficient if the user only aims to remove the tag from a few persons. +
**Alternative 2:** The command allows the user input to specify the persons from whom the tag should be removed. +
**Pros:** The user can remove the tags from a few people in one command. +
**Cons:** Implementation is more difficult as parsing of additional user input required. It is also unlikely for user +
to remove the tag from many persons at once, hence there is a function overlap with `edit` command.
