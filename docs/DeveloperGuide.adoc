...
*Cons:* The 42 cell design is fairly cumbersome, requiring the initialization of at least 42 cells. The class is limited in a way that it must have 42 cells, thus sometimes many cells are left unused, and there is a wastage of memory. Alternative 2: Use JTable to display individual dates.
...
1. Create a EventCalendar class that contains the month that will be shown, containing all the necessary information required for a monthly calender (i.e. dates, name of month).
...
EventCalendarClassDiagram
_Figure 4.9.1.1 : Class Diagram for `EventCalendar`_

MonthDateBuilderClassDiagram
