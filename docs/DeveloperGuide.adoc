...
A `PersonContainsTagPredicate` is created to check if a person contains the tags of interest.
[source,java]
----
@Override
...
The list is filtered and updated through the `updateFilteredPersonList` method in the `Model` component. As the person list is of a `FilteredList<>` type, filtering of the list is handled by Java's `FilteredList` class method, `setPredicate`.
