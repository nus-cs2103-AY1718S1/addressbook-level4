...
== Introduction
**BoNUS** is an address book application written in Java. This developer guide aims to provide a comprehensive guide for
developers to understand the processes behind the development of this application. Furthermore, with the aid of sequence
diagrams, we hope to better illustrate the architectural structure and methodology behind the development of **BoNUS**
for fellow developers for reference if needed. **BoNUS** is open for extension, should any developers be interested,
feel free to contact anyone of us in the `AboutUs` section.
...
_Figure 3.3.1 : Data Table View of phpMyAdmin (a MySQL visualization tool)_

image::phpmyadminAddColumn.png[width="700"]
_Figure 3.3.2 : Add New Column in phpMyAdmin_
...
_Figure 3.3.3 : Class Diagram for `PropertyManager`_
...
_Figure 3.3.4 : Sequence Diagram for Adding a Customize Property_
...
_Figure 3.3.5 : Notifications via System Tray_
...
_Figure 3.3.6 : Class Diagram for `Reminder`_
...
4. Have a list of reminders in `Event` class (Figure 3.37). Since `Event` can have multiple reminders, each reminder is
instantiated with an `Event` parameter in it such that a `Reminder` is tagged to an Event itself. Figure 3.38 depicts the
relationship between `Event` and `Reminder`.

image::EventClassDiagram.png[width="200"]
_Figure 3.3.7 : Class Diagram for `Event`_

image::EventReminderRS.png[width="300"]
_Figure 3.3.8 : Relationship of Event and Reminder_
...
_Figure 3.3.9 : Sequence Diagram for Adding an event (reminders are automatically added)_
