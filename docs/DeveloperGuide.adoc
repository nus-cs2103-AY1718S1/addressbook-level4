...
This developer's guide consists of the following sections shown in the table of contents below. Note that certain functions may not have been impleted into the current version of Planno yet. The number beside shows which version the function was implemented in. +
...
By: Hu Wanqing, Jiang Chunhui, Hou Denghao, Leon Chow Wen Hao      Since: `August 2017`
...
The _Sequence Diagram_ below shows how the components interact using event center for the scenario where the user issues the command `delete 1`.
...
The diagrams given below show the structure of whole logic component, and structure of commands in details. +
...
The following diagram shows the class structure of the Model component.
...
Below is a sequence diagram for executing a find command: `find t/friends`. It will find persons that have the tag `friends`.

image::findSequenceDiagram.png[width="800"]
_Figure 3.2.1: sequence diagram for find command_
...
The following sequence diagram shows how add event command works:
...
The following sequence diagram shows how the list event operation works:
...
The following sequence diagrams below, Figure 3.4.1 and Figure 3.4.2, show how the delete event operation works:

image::DeleteEventSD.png[width="800"]
_Figure 3.4.1: Logic interactions for `deleteE 1` command_

image::SDforDeletePersonEventHandling.png[width="800"]
_Figure 3.4.2 : Component interactions for `deleteE 1` command_

. The user enters a delete event command which is parsed from the `UI` to `Logic` by `CommandBox`.
. `Logic` has `LogicManager` use `AddressBookParser` to parse the user command.
. This results in a `Command` object which is executed by the `LogicManager`.
. The execution will affect the `Model` by deleting an event.
. `ModelManager` will raise an `AddressBookChangedEvent` to `EventsCenter`.
. `EventsCenter` will handle the changes to `Storage` and `UI` using the `handleAddressBookChangedEvent` method.
. The `Command` will be added to the `undoStack`.
. The result of the command execution is encapsulated as a `CommandResult` which is passed back to `CommandBox` in `UI`.

=== DeleteEvent mechanism

The delete event command is facilitated by the `LogicManager`. It supports `undo` and `redo` as it inherits from `UndoableCommand`. Other delete commands are implemented similarly.

The following sequence diagram below, Figure 3.5, show how the delete event operation works:

image::DeleteEventSequenceDiagram.png[width="800"]
_Figure 3.5: Logic interactions for `deleteE 1` command_
