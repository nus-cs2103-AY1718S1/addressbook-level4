...
**Alternative 1 (current choice):** Currently, the file has to be stored in /data directory. +
**Pros:** Easy to develop the logic as there is only one filepath . +
**Cons:** User will have to specifically place the file in the /data directory which does not really make sense. +
...
(For all link:#use-case[use cases] below, the *System* is the `AddressBook` and the *Actor* is the `user`, unless specified otherwise)
...
* 1b. The given person is a duplicate.
+
[none]
...
2.  AddressBook redoes the command
+
Use case ends.
...
2.  AddressBook undoes the command
+
Use case ends.
...
=== Use case: Finds person

*MSS*

1.  User requests to find person by name
2.  AddressBook finds the person
3.  AddressBook displays the list of person(s) with the keywords
+
Use case ends.
