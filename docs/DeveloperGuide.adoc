...
The Myth's Book is an address book dedicated for university students to store events, birthdays, school email
and home number  in addition to the existing fields of a normal Address Book. The Myth's Book is a `Command Line Input` based software that is coded
...
The `Birthdays` command is an undoable command and thus inherits from `UndoableCommand`.
...
image::BirthdaysMechanism2.PNG[width="800"]
