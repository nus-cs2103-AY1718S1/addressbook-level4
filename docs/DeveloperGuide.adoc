...
Rolodex is a desktop address book application. It is catered to computing professionals who prefer working without a mouse to manage their contacts more efficiently.

This guide will walk you through the design, implementation and development environment of Rolodex. This is the go-to page for all your information needs in understanding our application.
...
The fuzzy finding mechanism is powered by https://en.wikipedia.org/wiki/Levenshtein_distance[Levenshtein distance],
...
A `HashMap` is used for mapping actions to key combinations. When the user presses a key combination matching one that is assigned in the
`HashMap`, the corresponding action will be executed. An example of such an execution is given as shown:
...
----
Where the key combination for the action `FOCUS_PERSON_LIST` matches the `keyEvent` captured by the event filter.

NOTE: If an action that is not mapped to a key combination is called, a warning message will be displayed in the terminal.

NOTE: If a mapped key combination is invalid, an `IllegalArgumentException` is thrown and the application terminates.

==== Design Considerations

**Aspect:** Data structure for mapping actions to key combinations +
**Alternative 1 (current choice):** `HashMap` in a separate utility class +
**Pros:** Allows easy addition of an action-key combination pair. +
**Cons:** Keys are re-mapped whenever the `KeyListener` calls for the `HashMap`,
since the `HashMap` has to be instantiated and mapped at the same time. +
**Alternative 2:** Constant variables in the same class +
**Pros:** Constant values are only created once. +
**Cons:** The `KeyListener` class is less cohesive since it acts as a storage for key mappings in addition to handling key events.

**Aspect:** Exception handling for missing key mappings +
**Alternative 1 (current choice):** Catches the `NullPointerException` and display a warning message in the terminal +
**Pros:**  Does not flood the terminal with exception messages since the exception will be thrown for every key press. +
**Cons:** More difficult to locate the missing mapping. +
**Alternative 2:** Throw a `NullPointerException` +
**Pros:** May be easier to locate the missing mapping using the stack trace from the exception thrown. +
**Cons:** Less feasible as, since the number of mappings is currently reasonably small,
it is simpler to look through all the key mappings and locate the missing map. +
// end::shortcut[]
...
2.  AddressBook prompts the user for confirmation
3.  User confirms the deletion
4.  AddressBook clear the AddressBook
+
Use case ends.
...
4.  AddressBook edits the person with the new data
+
Use case ends.
...
* 2b2. AddressBook shows a filtered list of persons
+
[none]
...
+
[none]
*** 2b3a1. AddressBook shows an error message.
+
...
+
[none]
*** 2b3b1. AddressBook shows an error message.
+
Use case resumes at step 2b2.
* 2b4. AddressBook edits the person with the new data
+
Use case ends.
...
2.  AddressBook drops down a list of possible commands
3.  User requests to select a specific command in the list
4.  AddressBook appends the remainder of the command into the input box
+
Use case ends.
...
Use case resumes at step 2.
[none]
* 2b. AddressBook does not display any possible command
+
Use case ends.
...
[discrete]
=== Use case: Logging into the AddressBook with login credentials

*MSS*

1.  User opens the application
2.  AddressBook shows a prompt for the login credentials
3.  User requests to login with valid credentials
4.  AddressBook initializes with the user's data
+
Use case ends.
...
[none]
*** 2a1a. AddressBook creates a new account
+
Use case resumes at step 2.
[none]
*** 2a1b. AddressBook failed to create a new account
+
