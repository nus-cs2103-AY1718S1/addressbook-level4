...
* *(NOT SURE WHETHER TO INCLUDE THE REST)*
...
*(ANY MORE THINGS TO ADD HERE? It has the following capabilities:??)*

*(MENTION THAT CONTAG IS STILL IN DEVELOPMENT? MANAGE THEIR EXPECTATION THAT IT WILL STILL CHANGE?)*

*(MENTION THAT THIS IS AN OPEN SOURCE PROJECT? THEY ARE WELCOME TO CONTRIBUTE OR FORK TO DEVELOP THEIR OWN APPLICATION? OR SAY THAT THEY CANNOT PR, SOLELY FOR THE DEVELOPMENT OF THEIR OWN APPLICATION ONLY, AND HENCE THE EXPECTATION THAT CONTAG MAY CHANGE IF THEY FORK AT DIFFERENT TIME)*

*(ENSURE THEM THAT WE WILL ONLY MOVE ON TO THE NEXT VERSION WHEN EVERYTHING IS WORKING PROPERLY, SO NO MATTER WHEN THEY FORK, THEY WILL ALWAYS HAVE A WORKING COPY OF THE PROJECT)*
*(WE NEED TO MAKE SURE WE KNOW THE CONTEXT OF THE DEVELOPER GUIDE)*

*(WHERE DO WE MENTION THAT THIS IS V1.5? WE WANT TO DEVELOP UNTIL V2.0, AND HENCE THE USER STORIES THAT HAVE NOT BEEN IMPLEMENTED. MENTION THAT EVERYTHING IN THE USER STORIES WILL BE IMPLEMENTED BY V2.0?)*

*(MENTION THAT DEVELOPMENT WILL BE HALTED AT V2.0. SO AFTER THAT, CAN FORK TO DEVELOP THEIR OWN ADDRESS BOOK. NO MORE PR?)*
...
*(NOT SURE WHETHER TO MENTION THAT APPENDIX A IS CATERED TOWARDS BEGINNER DEVELOPER TO GET THEM STARTED)*
...
*(HOW ABOUT JAVAFX THAT REQUIRES MINIMUM UNDERSTANDING OF XML, JSON AND CSS?)*

=== Setting up the Project in Your Computer (PICTURES - BUT HOW MANY?)

*(SHOULD WE PUT A SINGLE SENTENCE HERE? AND OTHER PLACES AS WELL? THE SENTENCE WILL BE A REITERATION OF THE SUBHEADING ...)*

. Fork this repository and clone the fork to your computer *(WHEN CAN WE COMBINE TO ACTION INTO ONE POINT?)*
. Open IntelliJ IDEA
[NOTE]
...
. Run the `seedu.address.MainApp` and try a few commands *(REFER TO USER GUIDE? CAN WE SAY THIS? LINK?)*
. link:#testing[Run the tests] to ensure they all pass. *(DO WE NEED TO WRITE REFER TO SECTION ... FOR THE TYPES OF TESTS, ETC? SAME FOR ALL THE LINKS?)*

=== Configurations to Do Before Writing Code (PICTURES!!!)

==== Configuring the Coding Style

This project follows https://github.com/oss-generic/process/blob/master/docs/CodingStandards.md[oss-generic coding standards]. IntelliJ's default style is mostly compliant with ours *(CHANGE TO CONTAG'S?)*. However, it uses a different import order from ours *(CHANGE TO CONTAG'S?)*. To rectify, *(WHEN TO USE , AND WHEN TO USE : ? ANY WAY TO STANDARDISE?)*
...
Optionally, you can follow the <<UsingCheckstyle#, UsingCheckstyle.adoc>> document to configure Intellij to check style-compliance as you write code. *(FOR THIS KIND OF LINK, DO WE NEED TO PROVIDE CONTEXT (WHAT IT IS ABOUT) OR WHICH FOLDER IT CAN BE FOUND?)*

==== Updating Documentation to Match Your Fork (DELETE THIS PART? OR SET IT AS A TIP AND CHANGE THE LINK TO POINT TO OUR PROJECT?)
...
==== Setting up CI (TO BE DELETED? OR SET IT AS A TIP?)
...
2. Take a look at the section link:#suggested-programming-tasks-to-get-started[Suggested Programming Tasks to Get Started]. *(PUT THIS AT THE INTRODUCTION PART INSTEAD? BOTH?)*

== Design

*(WRITE SOMETHING HERE? WHAT IS DESIGN? WHY IS IT IMPORTANT FOR DEVELOPER TO KNOW ABOUT DESIGN?)*
...
Commands that are not undoable are implemented this way in _Figure 3.1.2_:
[source,java]
----
...
----
_Figure 3.1.2 : Code section in ListCommand.java_

With the extra layer, the commands that are undoable are implemented this way in _Figure 3.1.3_:
[source,java]
----
...
----
_Figure 3.1.3 : Code section in UndoableCommand.java_

Suppose that the user has just launched the application. The `UndoRedoStack` will be empty at the beginning.

As depicted in _Figure 3.1.4_: The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th person in the address book. The current state of the address book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).

image::UndoRedoStartingStackDiagram.png[width="800"]
_Figure 3.1.4 : UndoRedoStack Diagram 1_

As depicted in _Figure 3.1.5_: As the user continues to use the program, more commands are added into the `undoStack`. For example, the user may execute `add n/David ...` to add a new person.

image::UndoRedoNewCommand1StackDiagram.png[width="800"]
_Figure 3.1.5 : UndoRedoStack Diagram 2_
...
As depicted in _Figure 3.1.6_: We will pop the most recent command out of the `undoStack` and push it back to the `redoStack`. We will restore the address book to the state before the `add` command executed.

image::UndoRedoExecuteUndoStackDiagram.png[width="800"]
_Figure 3.1.6 : UndoRedoStack Diagram 3_

[NOTE]
If the `undoStack` is empty, then there are no other commands left to be undone, and an `Exception` will be thrown when popping the `undoStack`.

The sequence diagram in _Figure 3.1.7_ below shows how the undo operation works:

image::UndoRedoSequenceDiagram.png[width="800"]
_Figure 3.1.7 : Undo sequence diagram_
...
As depicted in _Figure 3.1.8_: The user now decides to execute a new command, `clear`. As before, `clear` will be pushed into the `undoStack`. This time the `redoStack` is no longer empty. It will be purged as it no longer make sense to redo the `add n/David` command (this is the behavior that most modern desktop applications follow).

image::UndoRedoNewCommand2StackDiagram.png[width="800"]
_Figure 3.1.8 : UndoRedoStack Diagram 4_

As depicted in _Figure 3.1.9_: Commands that are not undoable are not added into the `undoStack`. For example, `list`, which inherits from `Command` rather than `UndoableCommand`, will not be added after execution.

image::UndoRedoNewCommand3StackDiagram.png[width="800"]
_Figure 3.1.9 : UndoRedoStack Diagram 5_

The following activity diagram in _Figure 3.1.10_ below summarizes what happens inside the `UndoRedoStack` when a user executes a new command:

image::UndoRedoActivityDiagram.png[width="200"]
_Figure 3.1.10 : UndoRedoStack Activity Diagram_
...
For the Model component, `CommandIdentifier` is implemented this way in _Figure 3.2.1_:
[source,java]
----
...
----
_Figure 3.2.1: Code section in CommandIdentifier.java_
[NOTE]
A valid command identifier includes all existing command and their aliases.

For the Logic component, when the user key in help command in the command box, `HelpCommandParser` will be called. It parses user input that folows the help command, which may contain the optional command identifier argument, as shown in _Figure 3.2.2_:
[source,java]
----
...
----
_Figure 3.2.2: Code section in HelpCommandParser.java_

To accomodate this enhancement, the following method is added into `ParserUtil`. The method takes in user input string and return a `CommandIdentifier` object, as shown in _Figure 3.2.3_:
[source,java]
----
...
----
_Figure 3.2.3: Code section in ParserUtil.java_

Finally, the `CommandIdentifier` is passed into `HelpCommand` to do the command execution. The execute method in the `HelpCommand` is implemented as follows in _Figure 3.2.4_:
[source,java]
----
public CommandResult execute() {
...
----
_Figure 3.2.4: Code section in HelpCommand.java_

The `HelpCommand` execution is done in `AddressBookParser`. _Figure 3.2.5_ below shows the output of this function.

image::HelpEnhancementScreenShot.PNG[width="800"]
_Figure 3.2.5 : Output of `HelpCommand` execution_
...
When the LocateCommand is being executed, after checking that the target index is valid, a new `JumpToListRequestEvent` is posted by the `EventsCenter` class to its `EventBus`, as shown in _Figure 3.3.1_:
[source,java]
----
...
----
_Figure 3.3.1: Code section in LocateCommand.java_

The UI part `PersonListPanel` is in charge of handling this event (i.e.`JumpToListRequestEvent`) , and does so by scrolling to the contact of the corresponding index and selecting it, as shown in _Figure 3.3.2_:
[source,java]
----
...
----
_Figure 3.3.2: Code section in PersonListPanel.java_

The UI outcome of scrolling and selecting a `PersonCard` in the `PersonListPanel` is shown in Figure 3.3.3 below:

image::PersonListPanelSelect.png[width="400"]
_Figure 3.3.3 : UI outcome of scrolling and selecting a `PersonCard`_

When the constructor of `PersonListPanel` class is called, it will add a listener to the personListView which consists of a list view of each `PersonCard`. This listener ensures that a new `PersonPanelSelectionChangedEvent` will be raised when a new item in the list view is being selected. This can be shown below in _Figure 3.3.4_:
[source,java]
----
public class PersonListPanel extends UiPart<Region> {
...
----
_Figure 3.3.4: Code section 2 in PersonListPanel.java_

The `BrowserPanel` contains a `WebView` which displays a web page of the google map location of the selected person's address. It is able to do so as it is an event subscriber for the `PersonPanelSelectionChangedEvent` as shown below in _Figure 3.3.5_:
[source,java]
----
...
----
_Figure 3.3.5: Code section in BrowserPanel.java_
...
`Find Command` will only execute when the prefix of the parameters is given. Any other prefix as input will be considered as invalid input. Checks will be conducted via `FindCommandParser` which inherits from `FindCommand`, as shown in _Figure 3.4.1_.
...
----
_Figure 3.4.1: Code section in FindCommandParser.java_
...
Also,the input will first have to be tokenised in order to extract the prefix for for the search procedure. In absence of valid prefix ,an exception will be thrown, as shown in _Figure 3.4.2_.
[source,java]
----
...
----
_Figure 3.4.2: Code section 2 in FindCommandParser.java_
...
Upon any successful match, the particulars of the contact will be displayed. _Figures 3.4.3a to 3.4.3e_ below indicate the various outcomes of search function by  using `FindCommand` .


image::FindCommand_Name.PNG[width="400"]
_Figure 3.4.3a : Find by name [n/NAME]_

image::FindCommand_Phone.PNG[width="400"]
_Figure 3.4.3b : Find by phone [p/PHONE]_

image::FindCommand_Email.PNG[width="400"]
_Figure 3.4.3c : Find by email [e/EMAIL]_

image::FindCommand_Address.PNG[width="400"]
_Figure 3.4.3d : Find by address [a/ADDRESS]_

image::FindCommand_Tag.PNG[width="400"]
_Figure 3.4.3e : Find by tag [t/TAG]_
...
The index(es) selected must exist in the current result display window, as shown in _Figure 3.5.1_.
...
----
_Figure 3.5.1: Code section in CopyCommand.java_

The selected emails are stored in a string ArrayList, then re-formatted to remove square brackets around the ArrayList and change all commas to semi-colons before sending the result to system clipboard output, as shown in _Figure 3.5.2_ below. This ensures that the final output string copied to clipboard has maximum compatibility with formatting conventions in email applications.
...
----
_Figure 3.5.2: Code section 2 in CopyCommand.java_

_Figures 3.5.3a and 3.5.3b_ below show examples of copy command on display in the UI, showing the acceptable format for indexes, and the expected result. Note that the message will only be displayed if the message is already copied to the system clipboard.:


image::CopyCommand_single_index.PNG[width="400"]
_Figure 3.5.3a : Copy from a single contact_

image::CopyCommand_multiple_index.PNG[width="400"]
_Figure 3.5.3b : Copy from multiple contacts_

_Figure 3.5.4_ below shows an applied use of this command in an e-mail application--addresses automatically detected upon clipboard paste

image::CopyCommand_browser_result.PNG[width="400"]
_Figures 3.5.4: Applied use of command in email application_
...
----
_Figure 3.6.2: Code section in BrowserPanel.java_

The method `loadCalendar` will pass the calendar url to `BrowserPanel` to open up calendar website in `BrowserPanel`. The outcome of successful launching of calendar webpage in browser is shown in _Figure 3.6.3_:

image::CalendarCommand.png[width="800"]

_Figure 3.6.3: Output of `CalendarCommand.java` execution_
...
....
Continuous Integration (CI) is the process of automating the build and testing of code every time a team member commits changes to version contron
....
