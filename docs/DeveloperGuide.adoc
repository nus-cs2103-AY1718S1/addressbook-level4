...
==== Selecting a group
To select a group in circles, users can enter the command `selectgroup INDEX`, where INDEX is the index of the group in the group list panel. +
Alternatively, users can also choose to click on the group in the group list to select it.

Firstly, when a `selectgroup` command is entered in the Command Box, it enters the `SelectGroupCommandParser` class, which parses the index of the group that is selected.

Next, the `SelectGroupCommand` class will take in the Index parsed by `SelectGroupCommandParser`, and calls the `deleteGroup()` method in `Model`.
If the Index is valid, it will post a `JumpToGroupListRequestEvent` which includes the `targetIndex` of the group that is being selected.
The code implementation is shown below, and can be found in
{repoURL}/src/main/java/seedu/address/logic/commands/SelectGroupCommand.java[`SelectGroupCommand.java`]
[source,java]
----
...
Then, the `GroupListPanel` class in `UI` will handle the `JumpToGroupListRequestEvent`, and selects the `GroupCard` of the selected group.
It will also call the `setEventHandlerForSelectionChangeEvent()` method, which raises a `GroupPanelSelectionChangedEvent` event.
The code implementation is shown below, and can be found in
{repoURL}/src/main/java/seedu/address/ui/GroupListPanel.java[`GroupListPanel.java`]
[source,java]
----
...
----
Finally, the `GroupPanelSelectionChangedEvent` is handled by `ModelManager`, which will update the `filteredList` of persons, to display the group members in the group on the `PersonListPanel`.
The code implementation is shown below, and can be found in
{repoURL}/src/main/java/seedu/address/model/ModelManager.java[`ModelManager.java`]
[source,java]
----
...
+
[none]
** 3a1. Circles shows an error message.
+
...
2. Circles displays sorted list of persons according to specified field
...
2. Circles displays the person's details
3. User selects 'pin' option
4. Circles will now display chosen person at top of list
...
2. Circles filters out all persons without [tag]
3. Circles shows a list of all persons with [tag]

Use case ends.

[discrete]
=== Use case: Exporting contact data

1. User inputs 'export'
2. Circles prompts User to select a location on terminal to save the data file
