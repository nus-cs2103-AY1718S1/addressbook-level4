...
Within `ModelManager`, we mask `FilteredList` with a `SortedList` object, and call `sortFilteredPersonList` method within `Model`. This sets the comparator that been parsed by `SortCommandParser` into the `SortedList` filtered list. For each different way of sorting, a different comparator is being passed into `sortFilteredPersonList`. +

The following is a sequence diagram detailing the process of how `SortCommand` sorts the list of persons. +

image::SortSequenceDiagram.png[width="800"]

Below is the code fragment found in `Model`.
...
The diagram below details the sequence of how Rubrika handles the import command.
...
Firstly, the `ImportCommandParser` will create an new `ImportCommand` with a `FileInputSteam` containing the vCard file to be imported.

An `BufferedReader` object is then created within `ImportCommand` to read in the data within the vCard file.
The contents are then read in this manner;
