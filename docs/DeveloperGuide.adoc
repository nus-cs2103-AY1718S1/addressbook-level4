...
* The first line must contains the following values, *in this specific order*, corresponding to contact attributes, separated by tab: `name` `occupation `phone` `email` `address` `website `tag`
 ...
 * `NoSuchElementException`: Just as `AddCommand`, this happens when `name`, `occupation, `phone`, `email`, `address` or `website` is missing, and that specific entry will not be added.
...
_Figure 4.3 : Sequence diagram for deleteMultipleCommand_

Delete multiple contacts extends from undoable commands, and it can be undone using the undo function. By keying in the command word `deleteMul` along with `indexes`, the command word will be detected in the `AddressBookParser` class where the arguments (indexes in this case) will be passed over to the `DeleteMultipleCommandParser` class, as seen from figure 4.3 above.
