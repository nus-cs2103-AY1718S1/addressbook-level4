...
When the user calls ``‘schedule’ or ‘sche’` to schedule the contacts, the `Model` will receive a `schedule` from the `Parser`.
...
To ensure consistency across the user inputs for add command, the user would have to input p/ student/(STUDENT_NUMBER) parent/(PARENT_NUMBER)
