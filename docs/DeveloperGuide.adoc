...
An `Event` is created by `EventName`, `Date` and `Address` as the details of the event.
It also implements `ReadOnlyEvent` interface. The class diagram is shown in Figure 5.3.1 below.

image::eventclassdiagram.PNG[width="800"]
_Figure 5.3.1 Event Class Diagram_
...
From Figure 5.3.2 below, it is visible that `AddEventCommand` inherits from `UndoableCommand` which is an extra layer between abstract `Command` class in which the AddEventCommand
can be undoable, where the previous state of The Myth's Book before the event is saved.

image::AddEventDiagram.PNG[width="800"]
_Figure 5.3.2 AddEventCommand Logic Class Diagram_
...
The illustration of the execution is illustrated in Figure 5.3.3 below.

image::addeventmodelseqdiagram.PNG[width="800"]
_Figure 5.3.3 AddEventCommand Model Sequence Diagram_
