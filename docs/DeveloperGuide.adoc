...
If the user inputs `all` in the `INDEX` field, instead of removing a tag based on the list displayed in `Model`, the tag is removed from everyone in the `internalList` of `AddressBook`. The `TAG` is then deleted from the `UniqueTagList` in `AddressBook`. +

If the user inputs an `Integer` in the `INDEX` field, the tag is removed from the person as well as the `UniqueTagList` in the `AddressBook`. Thereafter, we re-sync the `Tag` Objects in `UniqueTagList` with all the `Tag` contained within the `Person` Object in the `internalList`. +
...
The export function is done by the `ExportCommand` class, and currently supports vCard files (.vcf) and XML files (.xml).
