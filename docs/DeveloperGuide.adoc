...
Whenever an action such as "Add", "Clear", "Delete", "Edit", "Email", "Import", "List", "Redo", "RemoveTag" & "Undo" is executed, the LoggingCommand method will be called and information of the action is recorded in ConnectUsLog.txt with a timestamp.

With slight modification to link:#fig3.1.2[Figure 3.1.2], we have the following diagram showing the Class Diagram on how LoggingCommand is implemented in the system.

image::LogCommandClassDia.png[]
_Figure 4.13.1 : Class Diagram of LoggingCommand function_
...
----
[NOTE]
Where "information" indicate the data that the user are editing with while "type of action" indicate the action taken by the user.
...
_Figure 4.14.1 : Sequence Diagram of the ClearLogCommand_
...
In this section, we explain how users can generation Quick Response(QR) Code for any contact available in the addressbook using the phone number of the contact.
...
_Figure 4.15.1 : Sequence Diagram of the QrCallCommand_
