...
_Figure 4.3 : Sequence diagram for `FindCommand`_
...
**Aspect:** Address check of  `FindCommand` +
**Alternative 1 (current choice):** Search by checking all keywords +
**Pros:** By checking each keyword individually, users will be able to retrieve all the contacts whose address is in a particular region (eg: Searching `Serangoon` will return all contacts with address `Serangoon`. +
**Cons:** Common keywords like `blk` or `street` may return a lot of contact results. +
**Alternative 2:** Find by checking entire address parameter as a whole +
**Pros:** By checking the address as a whole, users will be able to find the exact contact that they want without getting a ton of redundant results. +
**Cons:** It is possible to have multiple contacts with the same address in an office setting which makes checking address as a whole parameter less effective.
...
image::RecentlyDeletedSequenceDiagram.PNG[width="800"]
_Figure 4.5 : Sequence diagram for RecentlyDeletedCommand_

As we need to store the details of the deleted contacts, We will create a `LinkedList<ReadOnlyPerson> RecentlyDeletedQueue = new LinkedList<>();` to store the recently deleted contacts for up to the past 30 contacts.
In the `RecentlyDeletedCommand` class, we will convert all the contacts in `RecentlyDeletedQueue` into `String` to show the recently deleted contacts and their details in the `ResultDisplay` box.
The order of the contacts in the `RecentlyDeletedQueue` will also be reversed when shown in the `ResultDisplay` box, as we want to show the most recently deleted contact first.
// end::recentlydeleted[]
...
The input string is converted to a remark command object in the RemarkCommandParser class using the parse method:
...
image::sort.PNG[width="800"]
_Figure 4.5 : Sequence diagram for `SortCommand`_

Firsty, the user inputs a string "sort" which will then create a SortCommand instance which will then be excuted by the ModelManager class which will then sorts the list according to alphabetical order of the contacts names.
The new sorted list will then be returned and shown back to the user.
...
The `ThemeCommand` is used so as to allow the user to change the colour theme of TheYellowPage with the use of a single command. This provides the user with options to choose their preferred theme of TheYellowPage.
There are 2 themes to alternate from: `DarkTheme.css` and `LightTheme.css`. By keying in the command word `theme`, the command word will be detected by the `AddressBookParser` class and then a new instance of `ThemeCommand` class would be created.
By keying the command word `theme`, the theme of TheYellowPage will switch from `DarkTheme.css` to `LightTheme.css` and vice versa.

`ThemeCommand` is not an `UndoableCommand` and is implemented this way:
