...
One other important aspect to this implementation is that commands now have exclusive control over the list. If the user wishes to only work with blacklist, it is very much possible and all the current commands will only work on blacklisted persons. The persons not in this list will not be affected. For example, find command will only search among the blacklisted persons. This applies to all other commands. Nonetheless, if a person object is modified, the modification will be reflected across all other lists, given that the person exists in these various lists.
...
The command requires the index of the person the user wishes to update. However, indicating the index is optional if the person is the current selected person in the person list panel.
...
**Alternative 1 (current choice):** All three methods internally change the `Person` `Object`. Thus, each method will output the updated `ReadOnlyPerson` Object. The other methods will use obtain this and further update the `ReadOnlyPerson` `Object` +
**Pros:** Straightforward and simple to implement. +
...
`Setpath`, `Addpic` and `Delpic` commands facilitate in updating a debtor's display picture. `Setpath` command sets the `path` to a folder that contains `JPG` format images of the debtors. This folder resides in the user's workspace.
...
**Alternative 1 (current choice):** Pictures are not stored in Codii. Images are read from the path directory provided by the user
...
**Pros:** Far lesser hassle in transferring application content to another workstation +
