= Sales Navigator- Developer Guide
:toc:
:toc-title:
...
* exposes an unmodifiable `ObservableList<ReadOnlyPerson>` and an unmodifiable `ObservableList<ReadOnlyEvent` that can
be 'observed' e.g. the UI can be bound to this list so that the UI automatically updates when the data in the list change.
...
The events CRUD (Create, Read, Update, Delete) mechanism is very similar to the persons CRUD system.  Four commands
`AddEventCommand`, `FindEventCommand` (work in progress), `DeleteEventCommand`, `EditEventCommand` are used to execute their respective
functions through the `LogicManager`. +

image::EventsSequenceDiagram.png[width="800"]

Commands are first read from `CommandBox` in the UI Component which then calls upon the `LogicManager` in the Logic Component
to detect the type of command.
`LogicManager` then creates a `AddressBookParser` which uses `CheckCommandsParser` to detect any synonymous alternatives
for their respective commands and returns the specific command type if available. +

The specific command type is passed through its specific parser along with its arguments (E.g. `DeleteEventCommand` is parsed
via `DeleteEventCommandParser`). The specific parser creates a specific command to return to `LogicManager` (E.g.
`DeleteEventCommandParser` returns `DeleteEventCommand` with the index of the `Event` to be be deleted once executed). +

`LogicManager` proceeds to execute the received command. In this implementation, the Command will call upon
`ModelManager` in the Model component to do complete the respective CRUD operation onto its internal database.
(E.g. `DeleteEventCommand` results in `model.deleteEvent(event)` to delete the event at index indicated by
`DeleteEventCommandParser` from the event list in `AddressBook`. +

Once the model component has been updated, a signal is sent to the the `filteredEventList` which updates the UI
Component. +

Additionally, an `AddressBookChangedEvent` is also raised to the `EventsCenter` to indicate a change into so that
`XmlAddressBookStorage` in the storage component can begin saving the new information. It calls on
`XmlSerializableAddressBook` to serialize the new event list information and saves it to the data file via
`XmlFileStorage`.

==== Design Considerations

**Aspect 1:** Implementation of Storage +

**Alternative 1 (current choice):** Saving to a single XML file shared with contact data +
****
**Pros:**

* We do not need a separate data file to save event information. Additionally, reading data is only done once. +

**Cons:**

* Not very modular and intuitive. +

****

**Alternative 2:** Create a separate XML file to record events data +
****
**Pros:**

* Modular and intuitive to new developers. Manual additions to the data file will also be easier. +

**Cons:**

* Separate files are harder to manage.
****

**Aspect 2:** Implementation of Parsers +

**Alternative 1 (current choice): ** Creating individual parsers for a new function (E.g. `AddEventCommandParser` for `AddEventCommand`) +
****
**Pros:**

* Modular implementation where a parser only parses for a specific command.

* Issues will be easy to isolate and target.

**Cons**

* Repeated functionality with correponding parsers for Persons.

****

**Alternative 2: ** Creating a shared parser for events and persons which switch functionality depending on an input +
****
**Pros:**

* Removes the repeated functionality amongst the 4 pairs of parsers.

**Cons**

* Complex implementation which will be harder to maintain in the long run of development.

* More difficulty in debugging with newer changes.
...
// tag::timetable[]
=== Timetable Events
The timetable events schedule functions similarly to the events list on the left of the application. Upon starting the
application, after the model component initialises saved data to create its "memory-model" of the storage, `AddressBook`
uses its new functionality `getCurrentDate()` to retrieve today's date as indicated on the local machine and saves it
to retrieve the scheduled events for today. +

The implementation of the `EventList` used to store all the events in the model component has been modified to a `TreeMap`
to allow the retrieval of a sublist of events between any two dates. This is done through `getObservablesSublist()` which
uses the current date and the date that is one day after (possible due to the implementation of `addDays()` in `Date`. +

image::TimetableUpdate.png[width="800"]

This list is stored in the model component and will be updated accordingly if there are any changes to events made known
upon the signalling of `AddressBookChangedEvent`.
...
* through the `add` or `edit` command; OR

* through the GUI

==== Command-line implementation

When the image file location is specified in the `add` or `edit` command, the
 `AddCommandParser` and `EditCommandParser` parses the command and copy the
 file specified from the local file system to the
 `/src/main/resources/images` directory. The URL string of the copied image
 file is then stored as the `photoURL` String in the `Photo` field of a
 `Person`.

When the person's details are loaded in the `PersonPanel`, the `photoURL`
stored in the person is then loaded as an image and displayed in the graphic
UI.

==== GUI implementation

For every `PersonPanel`, there is an `Import Image` button which allows the
user to select a file from the local file system to import. Similar to the
command-line implementation, this file is then copied to the
`/src/main/resources/images` directory and then loaded in the graphic UI.
...
The sorting mechanism achieved by creating a new `SortCommand` and `SortCommandParser`. It is also facilitated by
...
After a series of method calls, the *model.sortPerson()* will eventually call the *sortPerson()* method in
`UniquePersonList`.
...
Comparators for sorting *Name* and *Tag* are the same, they do not have a custom comparator.
...
*Alternative 1*: Just sort all the different methods alphabetically.
...
*Alternative 2 (current choice)*: Have the sorting method to deal with such specificaitions.

*Pros*: Users is able to sort alphabetically automatically for all the sorting method provided.

*Cons*: Custom comparator will be required to compare the the sorting types that have a specific format.
...
The setting of relationship mechanism achieved by creating a new `SetRelCommand`, `SetRelCommandParser`
, `UniqueRelList` and `Relationship` classes. `SetRelCommand` and `SetRelCommandParser` classes resides in the `Logic
Component` facilitated by `UniqueRelList` and `Relationship` classes resides in the `Model Component`.
...
format by checking if it matches the regular expression for it. The overloaded constructor forms the relationship
the string of the other person's name and the relationship.
...
2. the indexes are the same.
3. the indexes are an unsigned integer.
4. the relationships are not empty strings or null for adding and deleting.

`SetRelCommandParser` returns a new instance of `SetRelCommand` that takes in two different indexes, an editedPerson
...
The implementation of the adding/deleting and clearing was inspired to be by my course mate, Sim Yan Yu
...
==== Design Considerations:
*Aspect*: Settng relationships for contacts
...
An example of the implementation checking the *add* command in `CheckCommandsParser` is as follows:
[source, java]
----
/**
...
[NOTE]
If the input command does not match with any of the *aliases/synonyms*, an exception will be thrown to notify the
user that the command entered is *invalid*.
