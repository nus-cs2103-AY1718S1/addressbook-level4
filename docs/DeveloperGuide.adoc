...
_Figure 4.2.1: Sequence diagram for the command `help add`_
...
The `ScheduleCommand` takes in 2 parameters: `ScheduleDate` and `Activity`. Therefore, the Model component of the application has been extended to include these two new classes. To make sure that users enter the correct DD-MM-YYYY format and valid date, `DateUtil` is created. `DateUtil` contains various methods to handle dates, including the methods shown below to check the validity of the input date.
