...
This `Predicate` is then used to filter the persons in the AddressBook.
...
This `predicate` is passed through `model.updateFilteredPersonList(predicate)` to update the `filteredPersonList`.
...
Meanwhile, the `ModelManager` posts a `FilteredPersonListChangedEvent` to the `EventsCenter`, causing the `StatisticsPanel` of the UI to update itself and reflect the new Statistics.
