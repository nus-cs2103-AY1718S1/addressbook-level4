...
To accommodate the execution of `ScheduleCommand`, the application's model component is extended to include `ScheduleDate` and `Activity`.

`DateUtil` is also created to make sure that users enter a valid data which follows DD-MM-YYYY format. The code snippet below shows how `DateUtil` checks the validity of an input date.
...
_Figure 4.3.3: UI display of a successful `ScheduleCommand` execution

==== Design Considerations

**Aspect:** Valid date format +
**Alternative 1 (current choice)**: DD-MM-YYYY is the only valid date format +
**Pros:** It is now possible to arrange the dates based on which date comes first. +
...
The implementation of `Comparable` interface for `Schedule` allows the activities scheduled with a person to be displayed from the smallest date value. The code extract in _Figure 4.4.1_ below shows how the `compareTo()` function is being defined in the `Schedule`.
...
The outcome of schedule multiple command execution is shown in _Figure 4.4.2_ below.

image::ScheduleCommandMultipleScreenShot.png[width="800"]

_Figure 4.4.2: UI display of an enhanced successful `ScheduleCommand` execution
...
**Aspect:** Data structure to support scheduling multiple activities with one `Person` +
**Alternative 1 (current choice)**: DD-MM-YYYY is the only valid date format +
**Pros:** It is now possible to arrange the dates based on which date comes first. +
**Cons:** Users have lesser flexibility in inputting dates. +
**Alternative 2:** Allowing users to key in dates in DD-MM format in addition to DD-MM-YYYY format  +
