...
`FindCommand` which reside in `Logic` will support the search of contact by any keywords.The find function only deals with existing parameters `Name`, `Email`, `Address`, `Phone`,`Tags` , `Activity` and `Country`. Such parameters inherit from `Person` which resides inside `Model`.Also,the keywords must be a full word match in order for successful execution of find command. _Figures 4.4.1 indicates the parsing of keywords and prefix  `FindCommandParser` once user keys in an input.
...
----
The component diagram in _Figure 4.4.3_ below shows how the find function works in the various component ,mainly `Logic`:
...
The component diagram in _Figure 4.6.3_ below indicates the compoenent diagram of the calendar function ,mainly `Logic`.
...
`EmailCommand` utilise `Desktop` to launch a email platform for users to send email to multiple contacts.User will have to key in index of the contacts
