...
The removeTag command removes a tag with the specified name from the address book. All the appearances of the tag in
the address book are removed. This command is implemented in the style of the `edit` command and `delete` command.
...
**Pros:** There is more flexibility and remarks do not have to be stated upon addition of Person to addressbook. +
**Cons:** More difficult to implement as new remark command and tests need to be created +
**Alternative 2:** Add in remarks together with other Person details such as address, phone, email, etc +
**Pros:** Easy to implement (implement spacing to create a new command class, just modify existing add command) +
**Cons:** The user will have to key in huge amount of information before adding into the addressbook. +

---

**Aspect:** Implement remark as distinct from tag +
...
Credits: Mechanism adapted from Yamgent https://github.com/se-edu/addressbook-level4/pull/599
// end::RemarkCommand[]
