...
// tag::tagremove[]
=== Removing a tag to multiple people
...
After the command word `t-remove` is parsed at the `AddresBookParser`, the remaining user input(indices in the list, tag to remove), in the example `t-remove 1 2 3 college friend`, will be passed to and parsed by `TagRemoveCommandParser`.
We also support multiple words as `tagName`, as long as the first word in `tagName` is not a number. +
This is because the key is to differentiate between indices and `tagName` is by identifying the last recognized number. If a `tagName` has its first word as a number, that word would be treated as index. +
In this case, as `3` is the last part that is numeric, `1 2 3` is treated as indices and `college friend` is treated as `tagName`. +
