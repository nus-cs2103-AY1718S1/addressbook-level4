...
|`* * *` |debt collector |indicate and update amount owed by a debtor |see how much a debtor owes
...
|`* * *` |debt collector |have link:#critical[critical] lists |quickly view which debtors require immediate attention
...
|`* *` |security-concerned debt collector |have a password protection to my account |have some security over confidential information
...
|`* *` |debt collector |have the ability to impose different periods of ban on my blacklisted clients |have clients automatically transferred out of that list after their ban
...
|`* *` |meticulous debt collector |see my client's progress bar which indicates their debt & the time remaining to pay it off |have a better visual representation of data
...
|`* *` |security-concerned debt collector |have 2FA authentication for login and every other important action(exporting contacts) |ensure that no one else is capable of accessing sensitive information
...
|`*` |debt collector |randomly generate a contact from the cleared list(clients who have paid their debts) |choose to chat up a random contact to talk them into getting a loan

|`*` |debt collector |use a file upload feature |upload signed documents to a specific client in the database

|`*` |debt collector |have a 'add-log' button attached to each client in the database |easily click it whenever I contact the client
|=======================================================================
...
....
An action that enables certain contacts to be displayed on application start-up
....
