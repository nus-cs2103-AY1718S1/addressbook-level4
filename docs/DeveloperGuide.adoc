...
The `upcomingbirthdayCommand` inherits from `Command` and does not supports undoing and redoing. When the user inputs the upcomingbirthday
, the `UI` calls the `execute()` method in the `LogicManager` Class. The `Logic` will then call for the `parseCommand()`
...
according to the chronological order of birthdays in the format DD-MM from the local date in the clock and generate a success message.
The following sequence diagram summarises what happens when the `upcomingbirthdayCommand` is implemented:
