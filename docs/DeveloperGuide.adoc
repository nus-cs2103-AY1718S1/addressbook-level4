...
----
The `HelpCommand` execution is done in `AddressBookParser`. Below shows the output of this function.
...
----
[NOTE]
A valid find command search keywords based on prefix  [n/NAME] or [p/PHONE] or [e/EMAIL] or [a/ADDRESS] or [t/TAG]
...
Examples of copy command on UI:
...
Figures 1 and 2 show the acceptable format for indexes, and the expected result. Note that the message will only be displayed if the message is already copied to the system clipboard.

image::CopyCommand_browser_result.PNG[width="300"]

Figure 3 shows an applied use of this command in an e-mail application--addresses automatically detected upon clipboard paste
