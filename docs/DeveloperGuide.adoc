...
**Cons:** However, it is inconvenient for users as they have to use a specific `commandWord` for a specific parameter.  +
**Alternative 2:** Accept any type of user input after `find`. +
**Pros:** It is more user-friendly as there are fewer requirements for user input. +
**Cons:** It would be difficult to implement because it is tedious to distinguish the user input as several parameters are `String` parameters.
// end::findcommand[]
...
process is executed. This command allows user to input `REMARK` field to a contact using the command format `remark INDEX r/[REMARK]`. If the `[REMARK]` is an
