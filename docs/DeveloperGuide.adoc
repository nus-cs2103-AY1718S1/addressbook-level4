...
By: `Team planno`      Since: `August 2017`      Licence: `MIT`
...
The find command accept two types of keyword: `name` and `tag`,

* `tag` keyword is identified by a prefix `t/`. +
e.g. `find t/friends` means to find any person that have a tag called `friends`
* anything that do not begin with `tag` is identified as name key word. +
e.g. `find friends` means to find any person whose name contains the keyword `friends`

Suppose now a user enters a find command `find t/friends` to find the persons that have the tag `friends`.
...
Below is a sequence diagram for doing command `find t/friends`
...
|`* * *` |user |add a person with link:#blank-information[blank information] |add him even if I do not know some details (e.g his address)
...
|`* * *` |lazy user |have auto complete when I type  |type less word

|`* *` |user who share my device with others |password to lock my address book |prevent others looking at it
...
|`* *` |user with many friends |add multiple people at one time |save my time for adding

|`* *` |user |sort contacts |view my list easily

|`* *` |user |tag my contacts |remember who they are through tags

|`* *` |security conscious user |log in with my id and password |keep my privacy

|`* *` |elder user |adjust font size |see more clearly
...
|`* *` |user |export contact details |share with others or transfer to other device

|`* *` |user |create filter using multiple tags |make a specific search using tags

|`*` |frequent user |listen some background music |address book happily

|`*` |user with many persons in the address book |sort persons by name |locate a person easily

|`*` |user |add pictures to contacts |remember who they are through pictures

|`*` |secretive user |alter whether information is private |make information open to everyone whenever I want

|`*` |user |drag person card |sort people into my own order

|`*` |user |select font style |more comfortable to read
