...
----
The component diagram in _Figure 4.4.3_ below shows the interactions within the Logic component for the execution of find command.
...
----
_Figure 4.4.2: Code section in CalendarCommand.java_

The component diagram in _Figure 4.4.3_ below indicates the interactions within the Logic component for the execution of calendar command.

image::CalendarCommandComponentDiagram.PNG[width="800"]
_Figure 4.4.3 : Clenader sequence diagram_



The  `BrowserPanel`  which reside in `UI` will handle the event of `ShowCalendarRequestEvent`
as shown in _Figure 4.4.4_:
[source,java]
----
public class BrowserPanel extends UiPart<Region> {
@Subscribe
...
----
_Figure 4.4.4: Code section in BrowserPanel.java_

The method `loadCalendar` will pass the calendar url to `BrowserPanel` to open up calendar website in `BrowserPanel`. The outcome of successful launching of calendar webpage in browser is shown in _Figure 4.4.5_:

image::CalendarCommand.png[width="800"]

_Figure 4.4.5: Output of `CalendarCommand.java` execution_
...
image::SequenceDiagramEmail.PNG[width="500"]
