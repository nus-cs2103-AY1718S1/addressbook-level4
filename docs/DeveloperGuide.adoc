...
The Classroom is a software address book that allows users to store contacts and manage them using a variety of features.
This guide provides information for you to set up and get started as a The Classroom developer, and is detailed enough to be useful to experienced developers as well.
Developers are recommended to be proficient in java before they make changes and update the existing code as there is heavy emphasis on specific java syntax and features.
...
The find feature allows you to find persons based on keywords given. These keywords could belong to any of
...
The following sequence diagram further illustrates how the find mechanism works.
...
**Alternative 2:** Introduce a conditional operation. +
...
**Alternative 1 (current choice):** Consolidate all the `Comparators` in the class `ComparatorUtil` and provide getter methods for them +
...
**Alternative 2:** Create `Comparators` in `ModelManager` and use them to sort +
...
**Alternative 1 (current choice):** When updating or deleting a person, get the old person's `Tags` and iterate through the whole contact list to find out if the `Tag` is still being used +
**Pros:** Easier to implement +
**Cons:** Slow run speed +
**Alternative 2:** Include a person list in `Tag` to hold information of the people who have that `Tag` and remove the `Tag` when there is no one left in the list. +
**Pros:** Fast run speed +
...
As seen above, execution of `AddTagCommand` is governed by the `addTag` method located in the `Model` interface and implemented by
`ModelManager`.

==== Design Considerations
**Aspect:** Execution of AddTag command  +
**Alternative 1 (current choice):** Accepts multiple `Tags` and `Indexes` as a single command input. +
**Pros:** Allows greater efficiency in adding multiple `Tags` instead of doing it singly. +
**Cons:** May pose an inconvenience for users to add `Tag` a to wide range of `Index`. +
**Alternative 2:** Allow range of `Index` to be accepted as parameters. +
**Pros:** Better efficiency in mass addition of `Tags` from persons that are listed consecutively. +
**Cons:** Pose the same inconvenience to users if the index of the persons specified are widely scattered. +
...
**Alternative 2:** Incorporate mechanism such as Stacks to store individual addition of `Tag`. +
...
As seen above, execution of `RemoveTagCommand` is governed by the `removeTag` method located in the `Model` interface and implemented by
`ModelManager`.

==== Design Considerations
**Aspect:** Execution of RemoveTag command  +
**Alternative 1 (current choice):** Accepts multiple `Tags` and `Indexes` as a single command input. +
**Pros:** Allows greater efficiency in removing multiple `Tags` instead of doing it singly. +
**Cons:** May pose an inconvenience for users to remove `Tag` from a wide range of `Index`. +
**Alternative 2:** Allow range of `Index` to be accepted as parameters. +
**Pros:** Better efficiency in mass removal of `Tags` from persons that are listed consecutively. +
**Cons:** Pose the same inconvenience to users if the person whose `Tag` to be removed are widely scattered. +
...
**Cons:** The use of the `Undo/Redo` command may not be applicable. +
**Alternative 2:** Incorporate mechanism such as Stacks to store individual removal of `Tag`. +
**Pros:** Complements the use of `Undo/Redo`. +
...
image::UpdatedRemarkSequenceDiagram.png[width ="900"]
...
To allow for easy user manipulation of the remarks of different persons, we provided an `addremark` and `removeremark` command to compliment the remark checklist +
...
**Alternative 1 (current choice):** Arraylist of remarks +
...
**Alternative 2:** Hashtable of remarks, with index as key +
...
**Alternative 1 (current choice):** Create a large stackpane to display the remarks on a separate pane +
...
**Alternative 2:** Keep remarks as a displayable field under person card, similar to tags +
...
**Alternative 1 (current choice):** Browserpanel to open a twitter page+
...
**Alternative 2:** Creating a messaging platform in the application itself +
...
|`* *` |user |undo/redo amendments made to the Contact List Pro |immediately revert to previous version if mistakes are made
...
|`*` |user with many persons in the Contact List Pro |sort persons by name |locate a person easily

|`*` |user |view recent contacts added/searched |keep a log of the operations done in the Contact List Pro
...
(For all use cases below, the *System* is the `Contact List Pro` and the *Actor* is the `user`, unless specified otherwise)
...
2.  Contact List Pro shows a list of persons
3.  User requests to delete a specific person in the list
4.  Contact List Pro deletes the person
+
Use case ends.
...
+
[none]
** 3a1. Contact List Pro shows an error message.
+
...
4.  Contact List Pro sets the person as a favourite contact
+
Use case ends.
...
4.  Contact List Pro adds the person into the group
+
Use case ends.
...
+
[none]
** 3b1. Contact List Pro shows an error message.
+
Use case resumes at step 2.
...
2.  Contact List Pro confirms the request, yes or no
3.  User enters password
4.  Contact List Pro requests for password again to confirm
5.  User enters password again
6.  Contact List Pro sets up password and stores data in a .xml file
7.  Contact List Pro requests for answer to a predetermined security question.
8.  User enters answer
9.  Contact List Pro stores security answer in a .xml file linked to the account
...
+
** 4a1. Contact List Pro shows an error message.
+
Use case resumes at step 2.

* 10a. User type wrong password
+
** 10a1. Contact List Pro reveals first and last letter of password.
+
** 10a2. User has forgotten the password
+
** 10a3. Contact List Pro prompts user to provide security answer
+
** 10a4. Contact List Pro check if it match the answer from .xml file
+
** 10a4. Contact List Pro reveals the password if correct
+
...
4.  Contact List Pro shows restricted list
5.  User requests to sort persons by address
6.  Contact List Pro shows a list of persons sorted by address
+
Use case ends.
...
2.  Contact List Pro shows a list of person
3.  User requests to edit a field, followed by new field details
4.  Contact List Pro edits specified contact's details
5.  Contact List Pro shows amended contact's details
+
Use case ends.
...
4.  Contact List Pro shows message for successful deletion
+
Use case ends.
...
2.  Contact List Pro shows a list of persons with fields containing given input
+
Use case ends.
...
* 2a. The list is empty.
+
** 2a1. Contact List Pro shows an error message.
+
Use case ends.
...
1.  User requests to remove specified tags from selected indexes or from the whole Contact List Pro.
2.  Contact List Pro shows a successful message and a list of updated contacts.
+
Use case ends.
...
+
** 2b1. Contact List Pro shows an error message.
+
* 2c. Invalid index is given.
+
** 2c1. Contact List Pro shows an error message.
+
Use case ends.
...
....
The text bar where user types in when Contact List Pro is opened up
....
