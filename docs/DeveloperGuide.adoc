...
The dynamic drop-down list is created using `org.controlsfx.control.textfield.TextFields` API.
...
Possible commands are checked against the user input and added to the HashSet `suggestedCommands` accordingly, which will be displayed as the drop-down list.
...
Requesting the default email client to open a new compose window is achieved with the `java.awt.Desktop` class. However, not all platforms support the `Desktop` API.
...
After confirming that the action can be supported on the platform, an URI is constructed using the recipient's email address and subject input by the user (if any).
...
The `SUBJECT` string must be percent-encoded, which means all the spaces must be replaced with `%20`. This is to avoid follow the URI syntax standard.
