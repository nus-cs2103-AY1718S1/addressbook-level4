...
|`* * *` |User with many contacts |Delete multiple contacts from the previous list starting from the bottom |Can remove multiple contacts at once
...
|`* * *` |User |List all persons in the Address Book |See all of my contacts
...
|`* * *` |User |Undo an undo |Restore the Address Book to its previous state before the undo

|`* * *` |User |Use the arrow keys to call previous commands used |

|`* * *` |User |Use the Address Book without saving it |Avoid any loss of data in the case I forget to save

|`* * *` |User |Exit the Address Book with a command |Exit without clicking the browser’s close button

|`* * *` |User |Use single-letter short forms of commands |Type less per command

|`* * *` |User |Delete contacts by tags |Delete the whole group at once

|`* * *` |User |Change the colour of the contact tag |Easily identify the tags
...
|`* *` |User |Save the birthdays of my contacts |Eliminate the need to remember my contact’s birthday

|`* *` |User |Be reminded of any upcoming birthdays without inputting any commands |Be constantly reminded about any upcoming birthdays without consciously doing so

|`* *` |User |List all contact’s birthdays with reference from today’s date |See all upcoming birthdays

|`* *` |User |Assign a profile photo for each contact |Remember how they look like

|`* *` |User |Enter the information of my contacts in a separate window |Use the add command without typing in a long sentence

|`* *` |User |Create a few personalized lists of contacts |Add specific people to these separate lists

|`* *` |User |Set appointments reminders with contacts |Remember when to meet my contact

|`*` |User with many persons in the address book |Sort persons by name |Locate a person easily

|`*` |User |Select from multiple UI themes |Change my UI's colour scheme
...
.  A novice who has never used an addressbook should have no problem using it.
...
.  User should be able to transfer contacts and information from other users if permitted.
...
* Cannot integrate contacts from user's active WhatsApp groups
...
* Users can browse by album instead of a plain song list
* Similar to browsing contacts by group instead of by individual
* Supports automatic retrieval of missing songs if user's local album is incomplete ("Show Complete Album")
* Analogous to automatic addition of unknown contacts from a user's active WhatsApp groups
