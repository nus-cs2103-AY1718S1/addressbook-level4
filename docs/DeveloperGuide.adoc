...
There are `two` modes in The Myth's Book which is firstly, `Persons Mode`, and secondly, `Events Mode`. +
The mode switching mechanism allows users to type in different specific commands based on the mode
that is being used in the software. The commands are categorised into mainly three groups: `General Commands`, `Person Commands`, `Event Commands`.
...
The `Person Commands` and `Event Commands` commands are basically controlled by a boolean variable `personListActivated`. +
...
`Person Commands`, `Event Commands`

Suppose a user is currently on the `Persons Mode` on The Myth's Book and he wants to use the `Events Mode` and the commands related to `Events`.

As such,
. User is currently able to use commands related to Persons such as the `AddCommand` or `EditCommand`.
. User is currently unable to use commands related to Events such as `AddEventCommand` or `DeleteEventCommand`.
. User is currently able to use general commands related to both Persons and Events such as `HelpCommand` or `SwitchThemeCommand`.

The execution of Mode Switching Mechanism is processed in this way:
