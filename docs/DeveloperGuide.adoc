...
This document is the developer guide for this application. You will firstly learn how to set up the project and the developing environment.
...
In addition, from the appendices, you will see the current user stories, use cases, requirements and some special glossaries defined for this document. We have a small product survey section for comparison between this application and other similar products.
...
The Route planner is done using the embedded webview component of JavaFX.

==== Design Consideration

**Aspect:** Google Map version +
**Problem:** We are not using the full version of Google Map. That is because the embedded browser doesn't support the requirement for running full version.
...
**Alternative 1 (current choice):** add indexes to each person and task, store linking people's index in task. +
...
[NOTE]
====
The value correspondence: 1 (super important), 2 (important), 3 (normal), 4 (trivial), and 5 (super trivial).
====
