...
Alternatives considered is to allow user input multiple details in each search. It may help to narrow down and shorten the list of contacts that will be returned, but it is not useful as user may only recall some details of a person only and thus not implemented . For example:
...
* There will be no results returned in this scenario as input phone number does not match the phone numbers in Contact 1 and 2 that contains `John`
