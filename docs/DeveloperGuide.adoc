...
**Cons:** Causes the file size of `displaypictures` folder to get too large too quickly if users has alot of contacts that he wishes to add a display photo to or if users add photos at a rapid rate or both. +
...
**Cons:** Overwriting the previous photo of the specified contact means that `undo` command for `addphoto` will not be executed correctly, as the previous image of that contact has already been overwritten.
// end::addphoto[]
