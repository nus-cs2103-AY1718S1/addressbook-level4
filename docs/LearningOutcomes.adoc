...
:repoURL: https://github.com/se-edu/residentbook-level4/tree/master
...
Note how the <<DeveloperGuide#architecture, Developer Guide>> uses events to communicate with components without needing a direct coupling. Also note how the link:{repoURL}/src/main/java/seedu/room/commons/core/index/EventsCenter.java[`EventsCenter.java`] acts as an event dispatcher to facilitate communication between event creators and event consumers.
...
Note how components of ResidentBook have well-defined APIs. For example, the API of the `Logic` component is given in the link:{repoURL}/src/main/java/seedu/room/logic/Logic.java[`Logic.java`]
...
Note how the ResidentBook app uses Java ``assert``s to verify assumptions.
...
* Add more assertions to ResidentBook as you see fit.
...
Note <<DeveloperGuide#logging, how the ResidentBook app uses Java's `java.util.log` package to do logging>>.
...
Add more logging to ResidentBook as you see fit.
...
Note how ResidentBook uses the `ReadOnly*` interfaces to prevent objects being modified by clients who are not supposed to modify them.
...
Analyze the ResidentBook code/design to identify,
...
Note <<UsingGradle#, how the ResidentBook app uses Gradle to automate build tasks>>.
...
Note <<UsingTravis#, how the ResidentBook app uses Travis to perform Continuous Integration>>. (https://travis-ci.org/se-edu/residentbook-level4[image:https://travis-ci.org/se-edu/residentbook-level4.svg?branch=master[Build Status]])
...
Note how our CI server <<UsingTravis#, Travis uses Coveralls to report code coverage>>. (https://coveralls.io/github/se-edu/residentbook-level4?branch=master[image:https://coveralls.io/repos/github/se-edu/residentbook-level4/badge.svg?branch=master[Coverage Status]]) After setting up Coveralls for your project, you can visit Coveralls website to find details about the coverage of code pushed to your repo. https://coveralls.io/github/se-edu/residentbook-level4?branch=master[Here] is an example.
...
The link:{repoURL}/src/test/java/seedu/room/commons/util/StringUtilTest.java[`StringUtilTest.java`]
class gives some examples of how to use _Equivalence Partitions_, _Boundary Value Analysis_, and _Test Input Combination Heuristics_ to improve the efficiency and effectiveness of test cases testing the link:../src/main/java/seedu/room/commons/util/StringUtil.java[`StringUtil.java`] class.
...
Consider the link:{repoURL}/src/test/java/seedu/room/storage/StorageManagerTest.java[`StorageManagerTest.java`] class.

* Test methods `prefsReadSave()` and `residentBookReadSave()` are integration tests. Note how they simply test if The `StorageManager` class is correctly wired to its dependencies.
* Test method `handleResidentBookChangedEvent_exceptionThrown_eventRaised()` is a unit test because it uses _dependency injection_ to isolate the SUT `StorageManager#handleResidentBookChangedEvent(...)` from its dependencies.

Compare the above with link:{repoURL}/src/test/java/seedu/room/logic/LogicManagerTest.java[`LogicManagerTest`]. Some of the tests in that class (e.g. `execute_*` methods) are neither integration nor unit tests. They are _integration + unit_ tests because they not only check if the LogicManager is correctly wired to its dependencies, but also checks the working of its dependencies. For example, the following two lines test the `LogicManager` but also the `Parser`.
...
* *Singleton Pattern* : link:{repoURL}/src/main/java/seedu/room/commons/core/EventsCenter.java[`EventsCenter.java`] is Singleton class. Its single instance can be accessed using the `EventsCenter.getInstance()` method.
* *Facade Pattern* : link:{repoURL}/src/main/java/seedu/room/storage/StorageManager.java[`StorageManager.java`] is not only shielding the internals of the Storage component from outsiders, it is mostly redirecting method calls to its internal components (i.e. minimal logic in the class itself). Therefore, `StorageManager` can be considered a Facade class.
* *Command Pattern* : The link:{repoURL}/src/main/java/seedu/room/logic/commands/Command.java[`Command.java`] and its sub classes implement the Command Pattern.
...
** Sub classes of link:{repoURL}/src/main/java/seedu/room/ui/UiPart.java[`UiPart`] (e.g. `PersonListPanel` ) act as 'Controllers', each controlling some part of the UI and communicating with the 'Model' (via the `Logic` component which sits between the 'Controller' and the 'Model').
...
* Note how some PRs in this project have been reviewed by other developers. Here is an https://github.com/se-edu/residentbook-level4/pull/147[example].
* Also note how we have used https://www.codacy.com[Codacy] to do automate some part of the code review workload (https://www.codacy.com/app/damith/residentbook-level4?utm_source=github.com&utm_medium=referral&utm_content=se-edu/residentbook-level4&utm_campaign=Badge_Grade[image:https://api.codacy.com/project/badge/Grade/fc0b7775cf7f4fdeaf08776f3d8e364a[Codacy Badge]])
