...
=== Adding a person: `addSchedule`

Adds a busy time span to a person to the address book +
Format: `addSchedule index d/BUSY_DAY st/START_TIME et/END_TIME

[TIP]
A person can have any number of busy time spans as long as the start time is no earlier than 0600 and the end time is no later than 2330. (including 0)
