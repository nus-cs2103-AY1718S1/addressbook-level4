...
* Only Persons matching all the keywords will be returned (i.e. `AND` search). e.g. `n/Hans n/Bo` will not return `Hans Gruber`, `Bo Yang`
* You may also click on tags to do a search for that tag. (ie. find t/CLICKED_TAG);
****
...
[NOTE]
The genre can only be either pop, dance or classic
...
[NOTE]
This is different from stopping, as it can be resumed playing by using the play background music command.
