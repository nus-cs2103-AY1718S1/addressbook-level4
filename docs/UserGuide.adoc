...
** Creation of meeting with multiple people at the same timing for an event is possible. In this case, multiple `Meeting Cards` will be created. A group meeting consists of meetings that have the same `NameMeeting`,`DateTime` and `Place` but with different person.
+
