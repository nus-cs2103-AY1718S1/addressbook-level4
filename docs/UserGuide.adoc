...
** A person can be favorited by including the parameter `f/` [Since v1.2] +
* *To add a display photo for a person*:
** A person can have a display photo imported from the location specified by `DISPLAY_PHOTO_FILE_PATH` [Since v1.5]
...
** You can remove a person's display photo by including the parameter `dp/` without specifying any file path after it. [Since v1.5]
...
+
Marks the third person a `favorite` contact, changes the existing display photo to the new file specified by `C:/Users/Keith/Pictures/superman.jpg` and edits the social media accounts to be
...
+
Edits the name of the second person to be `Clark Kent`, removes existing display photo and clears all existing tags.

.Edited the name, and removed display photo and tags from the second person
