...
// tag::remark[]
=== Overwriting a person's remark : `remark` `Since v1.2`
...
Person with empty remarks will display `(add a new remark)`. +
...
Overwrites the 1st person's remark.
* `find betsy` +
`remark 1 rm/Betsy is a female` +
Overwrites the 1st person's remark in the results of `find` command.
* `list` +
`remark 2 rm/` +
Overwrites `remark` from the 2nd person's in the address book and displays it as `(add a new remark)`.
...
// tag::home[]
=== listing entered commands : `home` `Since v1.5`

toggles between student details panel, statistic panel and graph panel with a home screen.
Format: `home`


[NOTE]
selecting a person by clicking while the display page is up will not cause the screen to change back. +
Only typing the `select` command would cause it to toggle.
...
* Omit optional fields(email, address, postal code and tags) when adding students
