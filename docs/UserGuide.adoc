...
:repoURL: https://github.com/CS2103T-T09-B1/main
...
=== Adding / Editing / Removing birthday to person: `birthday` (Since v1.2)
...
=== Sort all entries by age : `sortAge`    (Since v1.2)

Sorts all entries in the address book by their age, from the oldest to youngest. +
Format: `sortAge`

=== Sort all entries by birthday: `sortBirthday`    (Since v1.2)

Sorts all entries in the address book by their birthday, from 1st Jan to 31st Dec. +
Format: `sortBirthday`

=== Retrieving all persons having an existing tag : `retrieve`    (Since v1.2)
...
=== Tagging multiple persons : `tag`    (Since v1.2)
...
=== List the birthday of contacts in calendar mode (Coming in v2.0)
Able to view contacts in calendar mode so that users can look at who's birthday is coming up soon.
Contacts with birthday within a week will have a special colour or icon to notify user of upcoming birthdays.
