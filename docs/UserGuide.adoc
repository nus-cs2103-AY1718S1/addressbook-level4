...
=== Locating persons in address book: `find`
...
* The search will only select distinct persons. e.g. `han solo han solo` will only return `han solo`
...
* Only full words will be matched when less than 4 characters. e.g. `hans` will not match `Han` but `han` will match `Han`
