...
=== Adding a Person: `add`, `a` or `+` _(Since v1.3)_
...
=== Clearing All Entries : `clear` or `c` _(Since v1.3)_

Clears all entries from Rolodex. +
Format: `clear` or `c` +
Keyboard shortcut: kbd:[Ctrl+Shift+C]

=== Deleting a Person : `delete`, `d` or `-` _(Since v1.3)_
...
=== Editing a Person : `edit`, `e`, `change` or `modify` _(Since v1.3)_
...
=== Exiting the Program : `exit`, `quit`, `close`, `bye`, `esc` _(Since v1.3)_

Exits the program. +
Format: `exit` +
Keyboard shortcut: kbd:[Alt+F4]

=== Locating Persons in Rolodex: `find`, `f`, `filter` or `search` _(Since v1.4)_
...
=== Viewing Help : `help`, `h`, `assist` or `faq` _(Since v1.4)_

Format: `help` or `h` +
Keyboard shortcut: kbd:[F1]

=== Listing Entered Commands : `history`, `his` or `past` _(Since v1.3)_
...
// tag::list[]
=== Listing All Persons : `list`, `l`, `show` or `display` _(Since v1.3)_
...
// tag::newfile[]
=== Creating a New Rolodex Storage File: `new`, `n`, `touch` or `>` _(Since v1.3)_
...
// tag::openfile[]
=== Opening an Existing Rolodex Storage File: `open`, `o`, `cd`, `ls`, `<` _(Since v1.3)_
...
=== Redoing the Previously Undone Command : `redo` or `r` _(Since v1.3)_
...
=== Adding a Remark: `remark` or `note` or `rmk` or `comment` _(Since v1.4)_
...
=== Selecting a Person : `select` or `s` _(Since v1.3)_
...
=== Tagging a Person
Tagging a person can be done with the
link:#editing-a-person-code-edit-code-code-e-code-code-change-code-or-code-modify-code-em-since-v1-3-em[edit command].

=== Undoing Previous Command : `undo` or `u` _(Since v1.3)_
