...
* The filepath *must be suffixed with .xml* e.g. 'C:\', 'docs/', ...
* If specified directory is missing, it will be automatically created when exporting.
****

Examples:

* `export C:\exported\AcquaiNote.xml` +
Exports the address book into exported folder in C drive in xml file format.
