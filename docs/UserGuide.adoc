...
=== Locating persons by names or phone numbers or tags: `finds`

Finds persons who contain any of the given keywords. +
Format: `finds n/[MORE_KEYWORDS] or p/[MORE_KEYWORDS] or t/[MORE_KEYWORDS]`
...
=== Extra `options` menu (NEW)

Each user will have an extra list of options to choose from beside their name. +
Icon is similar to a down arrow.

=== Delete function from `options`(NEW)

Deletes the specified person from the address book. +
Function can be found from the new `options` feature.

=== Coming soon in v2.0

==== GoogleMap function from `options`

Show google maps data based on person's address. +
Function will be found from the new `options` feature.

==== Themes selection for myBook interface from `options`
...
=== Set important contacts from `important command`

Allows user to set important contacts to always appear at the top of the list +
Function will be found from the new `important` command feature
...
e.g. `find James Jake`
* *Find by names or phone numbers or tags* : `finds n/KEYWORD [MORE_KEYWORDS]` or `finds p/KEYWORD [MORE_KEYWORDS]` or `finds t/[KEYWORD] [MORE_KEYWORDS]` +
