...
* `ctrl` + `z` : Undo command
* `ctrl` + `y` : Redo command
* `ctrl` + `shift` + `d` : Clear command

=== Navigation/Selection Shortcuts

* `enter` : Selects the command box
* `esc` : Deselects the command box +
+
When the command box is not in selection: +

* `↑` or `↓` : Scrolls the contact list +
* `ctrl` + `d` : Deletes selected contact (not implemented yet)
// end::excludeNavbar[]
