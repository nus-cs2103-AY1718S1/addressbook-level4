...
*Postal codes must be appended to the end of the address text.* i.e. Suppose you are adding a new parcel into Ark and
...
to differentiate between the different parcels that will be be delivered to the same person. Tracking numbers also
...
numbers include two `R`s followed by nine digits and ending with `SG`. i.e. `RR123456789SG` +
...
*Autocomplete for prefixes and indexes* +
...
_Figure 5.21.4 : After filling in the command into the text field, pressing kbd:[Tab] will bring you
 to the first missing prefix_

image::autocompleteWithFullAddCommandAndOnePrefixCycle.PNG[width="400"]
_Figure 5.21.5 : Pressing kbd:[Tab] will cycle to the next missing prefix_
...
_Figure 5.21.6 : After filling in the command into the text field, pressing kbd:[Tab] will bring you
 to the first available index_

image::autocompleteWithStartOfSelectCommandCycle.PNG[width="400"]
_Figure 5.21.7 : Pressing kbd:[Tab] will cycle to the next available index_
Examples:
