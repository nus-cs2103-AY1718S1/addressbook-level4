...
****
* Retrieves all persons having the specified `TAGNAME` tag.
...
* `retrieve friends` +
Retrieves all persons with the "friends" tag in the address book.
...
Format: `tag INDEX,[MORE_INDEXES],... TAGNAME/[MORE_TAGNAMES]/...`

****
* Removes all `TAGNAME/[MORE_TAGNAMES]/...` tags from the persons at the specified `INDEX,[MORE_INDEXES],...`.
* The indexes refer to the index numbers shown in the most recent listing.
* The indexes *must be positive integers* `1, 2, 3, ...`
...
* All tags will be removed from the persons at the specified indexes if `TAGNAME/[MORE_TAGNAMES]/...` is left blank.
****
...
Format: `retag OLD_TAGNAME NEW_TAGNAME`

****
* Renames the existing `OLD_TAGNAME` tag to `NEW_TAGNAME`.
* All persons having the `OLD_TAGNAME` tag will be updated accordingly.
* The tag names *must be alphanumeric*
****
...
Renames the existing "enemies" tag in the address book to "friends".
