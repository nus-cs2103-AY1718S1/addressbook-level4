...
`Tab` key now automatically completes your incomplete command stub.
Use it to automatically generate a command 'skeleton' from an incomplete command word input.
Future enhancements will allow autocompletion of names, addresses and more.
...
A person can have an optional property known as remark now.
Add a remark or modify the remark of an existing person through `remark` command.

Examples:

* `list` +
`remark 1 r/Likes to swim` +
Attaches `Likes to swim` as remark to first person on the list.
