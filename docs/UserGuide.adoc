...
[NOTE]
====
Undoable commands: those commands that modify the address book's content. They include `add`, `delete`, `edit`, `note` and `clear`.
====

If you ever typed in a command by mistake, typed in `undo` next. +
This command will restore the address book to the state where the previous _undoable_ command was not executed. +

Here is when you `delete` a contact by accident: +

. image:Ui.png[UI, 40, 40, role = "right"] `>> delete 1`
. Type in +
image:Ui.png[UI, 40, 40, role = "right"]`>> undo` +

You should see that the effects of `delete 1` has been reverted.

The following are more examples to help you better understand the `undo` command:

* Failure to `undo` as there are no undoable commands executed previously. You will see an error message displayed like that in _Fig_.
...
. Type in +
`>> u` +
The `clear` command is reverted.
. Type in +
`>> undo` +
The `delete 1` command is reverted.
...
Now you realized you made a mistake when you entered the `undo` command. This command will revert the most recent `undo` command. +

Here is when you `delete` a contact and `undo` the `delete` by mistake: +

. image:Ui.png[UI, 40, 40, role = "right"] `>> delete 1`
. image:Ui.png[UI, 40, 40, role = "right"] `>> undo`
. Type in +
image:Ui.png[UI, 40, 40, role = "right"]`>> redo`

You should see that the `undo` command has been reverted and the contact remains deleted. Refer to _Fig_ to see that you have obtained the correct results.

The following are more examples to help you better understand the `redo` command:

* Failure to `redo` as there are no `undo` commands executed previously. You will see an error message displayed like that in _Fig_.
. Type in +
`>> delete 1`
. Type in +
`>> redo`
...
. Type in +
`>> undo` +
The `clear` command should be reverted.
. Type in +
`>> undo` +
The `delete 1` command should be reverted.
. Type in +
`>> redo` +
The `delete 1` command should be reapplied.
. Type in +
`>> redo` +
The `clear` command should be reapplied.
// end::undoredo[]
...
If you want to close the app, simply type in: +
image:Ui.png[UI, 40, 40, role = "right"]`>> exit` +
*DummyName* will be closed and all contacts will be saved.
