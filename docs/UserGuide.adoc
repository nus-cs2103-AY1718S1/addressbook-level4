...
=== Setting a person(s) as "favourite" : `t-add` _[Since v1.3]_

Adds favourite tag to specified person(s) from the address book.

Format: `t-add INDEX... favourite`

****
* Sets person(s) at the specified `INDEX` as favourite.
* The `favourite` can be used interchangeably with `fav`, and words containing `fav`.
* The `favourite` tag is shown as a coral shaped circle in front of the tag list.
* The index refers to the index number shown in the most recent listing, multiple indices are allowed.
* The index *must be a positive integer* 1, 2, 3, ...
****
...
* `list` +
`t-add 2 3 favourite` +
Sets 2nd and 3rd person in the address book as favourite
* `list` +
`t-add 2 3 fav` +
Sets 2nd and 3rd person in the address book as favourite
...
* The index refers to the index number shown in the most recent listing, multiple indices are allowed.
* The index *must be a positive integer* 1, 2, 3, ...
* If no index is inputted, the provided [TAG] from all person(s) in the current list with the [TAG] will be removed.
* The [TAG] *must not have its first word as number*.
****
...
* `t-remove friends` +
Removes the tag `friends` from all person(s) with the tag `friends` in the address book

=== Removing a person(s) from favourite : `t-remove` _[Since v1.2]_

Removes `favourite` tag from specified person(s) from the address book.

Format: `t-remove INDEX... favourite`

****
*Removes person(s) at the specified `INDEX` from `favourite`.
* The `favourite` can be used interchangeably with `fav`, and words containing `fav`.
* If no index is inputted, the tag `fav` from all person(s) in the current list with `fav` will be removed.
* The index *must be a positive integer* 1, 2, 3, ...
****
...
* `list` +
`t-remove 2 3 fav` +
Removes the tag favourite from the 2nd and 3rd person in the address book

* `t-remove fav` +
Removes the tag `fav` from all person(s) with the the tag `fav` in the address book
...
// end::undoredo[]
// tag::bin[]
=== Listing all persons removed: `bin-list` _[Since v1.5]_

If you are worrying about deleting people wrongly, it would be a good way to see if they are in recycle bin.

You can see a list of persons removed from the address book.
...
* `bin-list` +
`bin-delete 1 2` +
Delete the 1st and 2nd person in the bin-list
...
>>>>>>> 1d15ad554d4924e835f96d292743a4a7a7c525e4
