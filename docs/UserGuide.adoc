...
****
* Remark the person at the specified `INDEX`. The index refers to the index number shown in the listing. The index *must be a positive integer* 1, 2, 3, ...
* It only can add the remark to the new person after I run the mainApp.
* You can remove all the person's tags by typing `remark INDEX r/` without specifying any tags after it.
****

Examples:

* `remark 1 r/Likes to drink coffee.` +
Edits the remark for the first person who are not in the exisiting list and  add the remark the new person to Likes to drink coffee.
* `remark 1 r/` +
Removes the remark for the first person.
