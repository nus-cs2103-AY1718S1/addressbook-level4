...
[NOTE]
Password cannot be copy-pasted into command box. It has to be typed out manually.
...
1) Type `login` in the command box. +
2) Enter username and password in the respective fields +
3) Press `Enter` on keyboard or click the `Log in` button +
