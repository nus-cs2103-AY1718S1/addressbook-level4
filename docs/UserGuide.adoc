...
Besides managing contacts, Death Note brings entertainment to you by having aliases for its commands and the entertainment
it brings is only limited by your creativity.
...
=== Add a person: `add` / `a` / `curse` _[Since v1.2]_
...
=== Add a remark : `remark` _[Since v1.1]_
//tag::remark[]
...
=== Edit a person : `edit` / `e` / `mutate` _[Since v1.2]_
...
=== Find a person: `find` / `f` / `summon` _[Improved since v1.3]_
...
=== Create new DeathNote : `new` / `n` _[Since v1.4]_
...
=== Open existing DeathNote : `open` / `o` _[Since v1.4]_
...
//tag::brem[]
=== View birthday reminder : `reminders` / `rem` _[Since v1.4]_
...
//tag::theme[]
=== Change theme : `theme` _[Since v1.2]_
...
=== Sorting Behavior _[Since v1.3]_
