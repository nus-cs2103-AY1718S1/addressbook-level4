...
Such details include recipient's names, delivery due dates, locations, and tags.
Ark is also able to help you plan out your delivery route based using google maps
...
of every parcel and bases its route planning base on that.
...
Address book comes with tab autocompletion which is able to complete a command for you
without requiring you to type out the command fully. +
To make use of this feature, simply key in the first few characters of the command you with to enter and press the `Tab` key.
Address book will then fill in the rest of the command for you.
Then press `Enter` key to enter the command.
