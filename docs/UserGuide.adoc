...
[TIP]
You can use `ab` for `addressbook` and `tm` for `taskmanager`
...
Want to find a task? The find feature allows you to find a task using descriptions or deadline.

To find task whose description contain any of the given keywords, use +
Format: `find KEYWORD [MORE_KEYWORDS]`

To find task by deadlines, use +
...
Want to go back to the default list of task after a search? Simply use the list command:
