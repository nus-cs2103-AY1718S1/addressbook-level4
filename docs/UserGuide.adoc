...
Shows a list of all persons in the address book, sorted by the specified sort order or default sort order. +
...
Adds a profile picture to the specified person in the address book. +
...
=== Adding a remark: `remark` or `note` _(Coming in V2.0)_

Adds a remark to the specified person in the address book. +
Format: `remark INDEX [r/REMARK]` or `note INDEX [r/REMARK]`
