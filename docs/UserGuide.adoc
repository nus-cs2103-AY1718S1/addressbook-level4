...
==== Stalking a person: `stalk`

To view the social media feed of a contact, use the `stalk` command.
====
Command Format: `[red]#stalk# [green]#INDEX [SOCIAL_MEDIA_PLATFORM]#` +
Alias: `[red]#s#`
====
...
e.g. `-1`, `0.1`, `abc`
|Ensure that the specified [green]`INDEX` is a *positive integer* that is *smaller or equal to the total number of contacts in the person list*.
...
a|Ensure that the selected user has an associated account for the `SOCIAL_MEDIA_PLATFORM` specified. The social media accounts associated with
a user can be found in the person list.

.Contact information in a person list, with social media accounts highlighted in orange
...
* `[red]#find# [green]#john#` +
`[red]#sort# [blue]#-recent#` +
lists all the person that contain the keyword `john`, sorted based on when they were last added, updated, or stalked.
