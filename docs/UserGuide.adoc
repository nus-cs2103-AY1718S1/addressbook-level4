...
|===
| COMMAND | FORMAT
.2+| Add | `add n/NAME p/PHONE_NUMBER e/EMAIL a/ADDRESS r/REMARK [t/TAG]...`
         | *E.g.* `add n/James Ho p/22224444 e/jamesho@example.com a/123, Clementi Rd, 1234665 r/Loves Tea t/friend t/colleague`
.2+| Alias | `alias [ALIAS COMMAND]`
         | *E.g.* `alias unfriend delete 1`
.2+| Clear | `clear`
         | *E.g.* `clear`
.2+| Delete | `delete INDEX`
         | *E.g.* `delete 3`
.2+| Edit | `edit INDEX [n/NAME] [p/PHONE_NUMBER] [e/EMAIL] [a/ADDRESS] [r/REMARK] [t/TAG]...`
         | *E.g.* `edit 2 n/James Lee r/Loves Cake`
.2+| Find | `find PREFIX/KEYWORD [MORE_PREFIX/KEYWORDS]...`
         | *E.g.* `find James Jake`
.2+| List | `list`
         | *E.g.* `list`
.2+| Help | `help`
...
.2+| History | `history`
         | *E.g.* `history`
.2+| Music | `music [MUSIC COMMAND] [GENRE]`
         | *E.g.* `music play pop` +
         *E.g.* `music stop`
.2+| Radio | `radio [RADIO COMMAND] [GENRE]`
         | *E.g.* `radio play pop` +
         *E.g.* `radio stop`
...
.2+| Undo | `undo`
         | *E.g.* `undo`
.2+| Unalias | `unalias ALIAS`
         | *E.g.* `unalias unfriend`
|===
