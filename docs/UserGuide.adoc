...
Lists all the commands that the user has entered in reverse chronological order. +
Format: `history`
...
`undo` +
`undo` +
The `undo` command entered right after `clear` reverses the `clear` command. +
The next `undo` command reverts the `delete` `1` command, i.e. reverting the next Bluebird address book contact
modifying command.
