...
=== Adding a person: `add` `create` `put` _[Since v1.1]_
...
=== Listing all persons : `list` `show` `all` _[Since v1.1]_

Shows a list of all persons in the address book. +
Format: `list` or `show` or `all`

=== Editing a person : `edit` `update` `modify` _[Since v1.1]_
...
=== Locating persons by different attributes: `find` `search` `get` _[Since v1.2]_
...
=== Deleting a person : `delete` `remove` `-` _[Since v1.2]_

Deletes a group of people from the address book. +
Format:`delete I/INDEX INDEX ...`
(or)`remove I/INDEX INDEX ...`
(or)`- I/INDEX INDEX ...`
(or)`delete n/Name `
(or)`remove n/Name`
(or)`- n/Name`
...
****
* Deletes the people with given name or at these given specified `INDEX`.
...
=== Find the location of a person using Google Map: `m-show` _[Coming in v2.0]_
Format: `s-show INDEX`
...
=== Find the route from user's current location to the person seleted: `m-route` _[Coming in v2.0]_
Format: `s-route INDEX ADDRESS`
...
=== Selecting a person : `select` `choose` _[Since v1.1]_
...
=== Listing entered commands : `history` `record` _[Since v1.1]_
