...
* Parameters can be typed in any order e.g. if the command specifies `n/NAME p/PHONE_NUMBER`, `p/PHONE_NUMBER n/NAME` is also acceptable.
====
...
Format: `help` (or press `F1` on the keyboard)
...
[TIP]
Contacts will be sorted by creation time (increment)
...
Format: `config --add-property s/SHORT_NAME f/FULL_NAME [m/MESSAGE r/REGULAR_EXPRESSION]`
