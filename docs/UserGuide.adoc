...
.  Type in +
image:Ui.png[UI, 40, 40, role = "right"]`>> filter colleague`
.  Press `Enter` and you will see a list of persons who are tagged with `colleague`
.  Type in +
image:Ui.png[UI, 40, 40, role = "right"]`>> filter friends`
.  Press `Enter` and you will see a list of persons who are tagged with both `friends` and `colleague`
