...
Shows a list of persons in the address book containing any of the input`tag` +
Format: `list TAG1 TAG2...` +
 +
Examples: +
`list colleagues friends` +
Lists all contacts with the tag `colleagues` and/or `friends`.
...
Sets random colors to all tags OR remove colors. +
Format: `tagcolor random/off` +
Shorthand Format: `tc random/off`

Sets a color to a specific tag type. +
...
To see all the colors that can be specified, refer from +
...
Take note the name is case-sensitive.

====

=== Adding/Removing Remarks to a person : `remark`

Adds/Remove a remark from an existing person in the address book. +
...
* Right: Activate Add Command hack if conditions are right +
Condition 1: Add Command hack will trigger only if the caret is at the end of the line +
...
* *Alt* : Shifts text cursor all the way to the left
* *Control* : Shifts text cursor all the way to the right
* *Right* : Add Command Hack will trigger if the caret is at the end of the line +
