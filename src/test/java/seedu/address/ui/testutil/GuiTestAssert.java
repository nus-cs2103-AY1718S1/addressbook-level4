...
public class GuiTestAssert {
    ...
    public static void assertCardDisplaysPerson(ReadOnlyPerson expectedPerson, PersonCardHandle actualCard) {
        ...
        assertEquals(expectedPerson.getAppointment().value, actualCard.getAppointment());
        assertEquals(expectedPerson.getTags().stream().map(tag -> tag.tagName).collect(Collectors.toList()),
                actualCard.getTags());
        assertEquals(expectedPerson.getGroups().stream().map(group -> group.groupName).collect(Collectors.toList()),
