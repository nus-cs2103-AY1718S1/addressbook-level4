...
public class AddEventParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Event expectedEvent = new EventBuilder().withName(VALID_EVENT_A_NAME).withDate(VALID_EVENT_A_DATE)
                .withAddress(VALID_EVENT_A_ADDRESS).build();

        // multiple names - last name accepted
        assertParseSuccess(parser, AddEventCommand.COMMAND_WORD + EVENT_NAME_B_DESC
                + EVENT_NAME_A_DESC + EVENT_DATE_A_DESC
                + EVENT_ADDRESS_A_DESC, new AddEventCommand(expectedEvent));

        // multiple date - last date accepted
        assertParseSuccess(parser, AddEventCommand.COMMAND_WORD
                + EVENT_NAME_A_DESC + EVENT_DATE_B_DESC + EVENT_DATE_A_DESC
                + EVENT_ADDRESS_A_DESC, new AddEventCommand(expectedEvent));

        // multiple address - last date accepted
        assertParseSuccess(parser, AddEventCommand.COMMAND_WORD
                + EVENT_NAME_A_DESC + EVENT_DATE_A_DESC + EVENT_ADDRESS_B_DESC
                + EVENT_ADDRESS_A_DESC, new AddEventCommand(expectedEvent));
    }
