...
public class AddEventCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Event expectedEvent = new EventBuilder().withTitle(VALID_TITLE_SOCCER).withTiming(VALID_TIMING_SOCCER)
                .withDescription(VALID_DESCRIPTION_SOCCER).build();

        // multiple titles - last title accepted
        String event1 = AddEventCommand.COMMAND_WORD + TITLE_MIDTERM + TITLE_SOCCER + TIMING_SOCCER
                + DESCRIPTION_SOCCER + TIMING_SOCCER;
        assertParseSuccess(parser, event1, new AddEventCommand(expectedEvent));

        // multiple timings - last timing accepted
        assertParseSuccess(parser, AddEventCommand.COMMAND_WORD + TITLE_SOCCER + TIMING_MIDTERM + TIMING_SOCCER
                + DESCRIPTION_SOCCER, new AddEventCommand(expectedEvent));

        // multiple descriptions - last description accepted
        assertParseSuccess(parser, AddEventCommand.COMMAND_WORD + TITLE_SOCCER + TIMING_SOCCER + DESCRIPTION_MIDTERM
                + DESCRIPTION_SOCCER, new AddEventCommand(expectedEvent));
    }
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + VALID_TITLE_SOCCER + TIMING_SOCCER
                + DESCRIPTION_SOCCER, expectedMessage);
        // missing timing prefix
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + TITLE_SOCCER + VALID_TIMING_SOCCER
                + VALID_TIMING_MIDTERM, expectedMessage);
        // missing email prefix
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + TITLE_SOCCER + TIMING_SOCCER
                + VALID_DESCRIPTION_SOCCER, expectedMessage);
        // all prefixes missing
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + VALID_TITLE_SOCCER + VALID_TIMING_SOCCER
                + VALID_DESCRIPTION_SOCCER, expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        // invalid title
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + INVALID_TITLE + TIMING_SOCCER + DESCRIPTION_SOCCER,
                Title.MESSAGE_TITLE_CONSTRAINTS);

        // invalid timing
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + TITLE_SOCCER + INVALID_TIMING + DESCRIPTION_SOCCER,
                Timing.MESSAGE_TIMING_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, AddEventCommand.COMMAND_WORD + INVALID_TITLE + INVALID_TIMING + DESCRIPTION_SOCCER,
                Title.MESSAGE_TITLE_CONSTRAINTS);
    }
}
